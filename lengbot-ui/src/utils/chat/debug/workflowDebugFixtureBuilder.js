...
function advancedNodeEvent(nodeId, type, label, extra = {}) {
  ...
  if (type === 'retrieval') {
    base.outputs = {
      ...
      results: [{ documentName: 'workflow-debug.md', score: 0.91, content: '高级拼装需要生成合法图和完整轨迹。' }],
    }
...
function appendAdvancedLoop(nodes, edges, events, previousExitNodeId, x, stepIndex, index) {
  ...
  edges.push(edge(previousExitNodeId, startId), edge(startId, scriptId), edge(scriptId, endId))
  events.push(
    {
      type: 'workflow_node_start',
      nodeId: groupId,
...
function buildLinearBasicFixture() {
  ...
  const edges = [edge('start', 'llm-summary'), edge('llm-summary', 'end')]
  const events = buildEvents([
    {
      nodeId: 'start',
      nodeType: 'start',
      nodeLabel: '开始',
      outputs: { query: '总结 LengBot Debug 能力' },
      message: '工作流已启动',
    },
    {
      nodeId: 'llm-summary',
      nodeType: 'llm',
      nodeLabel: '大模型总结',
      input: { prompt: '总结 LengBot Debug 能力' },
      outputs: { text: 'Debug Lab 用于验证前端渲染链路。' },
      durationMs: 860,
    },
    {
      nodeId: 'end',
      nodeType: 'end',
      nodeLabel: '结束',
      outputs: { answer: 'Debug Lab 可复现工作流渲染问题。' },
      durationMs: 8,
    },
  ])
...
function buildRagAnswerFixture() {
  const nodes = [
    node('start', 'start', 80, 160),
    node('retrieval-docs', 'retrieval', 300, 146, { label: '检索 Debug 文档', knowledgeName: 'LengBot Docs' }),
    node('llm-answer', 'llm', 560, 146, { label: '生成答案', modelName: 'Debug Mock Model' }),
    node('end', 'end', 820, 160),
  ]
  const edges = [edge('start', 'retrieval-docs'), edge('retrieval-docs', 'llm-answer'), edge('llm-answer', 'end')]
  const events = buildEvents([
    { nodeId: 'start', nodeType: 'start', nodeLabel: '开始', outputs: { query: '工作流 Debug 如何测试父子节点？' } },
    {
      ...
      outputs: {
        ...
        results: [
          {
            documentName: 'Debug Lab 扩展设计.md',
            score: 0.94,
            content: '工作流 Tab 应覆盖父子节点、容器节点、轨迹回放。',
          },
...
function buildConditionBranchFixture() {
  ...
  const events = buildEvents([
    ...
    {
      nodeId: 'condition-score',
      nodeType: 'condition',
      nodeLabel: '置信度判断',
      input: { score: 0.92 },
      outputs: { matched: true, branch: 'high' },
      nextNodeId: 'llm-high',
      durationMs: 14,
    },
    {
...
function buildBatchBasicFixture() {
  ...
  const events = [
    ...pairEvent(
      { nodeId: 'start', nodeType: 'start', nodeLabel: '开始', outputs: { items: ['问题 A', '问题 B'] } },
      0
    ),
    {
      type: 'workflow_node_start',
    ...
    ...pairEvent(
      {
        nodeId: 'batch-script-a',
        nodeType: 'script',
        nodeLabel: '分析问题 A',
        parentNodeId: 'batch-1',
        input: { item: '问题 A' },
        outputs: { summary: 'A 的处理结果' },
        durationMs: 210,
      },
      3
    ),
    ...pairEvent(
      {
        nodeId: 'batch-script-b',
        nodeType: 'script',
        nodeLabel: '分析问题 B',
        parentNodeId: 'batch-1',
        input: { item: '问题 B' },
        outputs: { summary: 'B 的处理结果' },
        durationMs: 240,
      },
      4
    ),
    ...pairEvent(
      {
        nodeId: 'batch-1_batch_end',
        nodeType: 'batch_end',
        nodeLabel: '并行结束',
        parentNodeId: 'batch-1',
        outputs: { merged: ['A 的处理结果', 'B 的处理结果'] },
        durationMs: 8,
      },
      5
    ),
...
function buildLoopBasicFixture() {
  ...
  const events = [
    ...pairEvent(
      { nodeId: 'start', nodeType: 'start', nodeLabel: '开始', outputs: { items: ['第一轮', '第二轮'] } },
      0
    ),
    {
      type: 'workflow_node_start',
...
function buildFailureRetryFixture() {
  ...
  const edges = [edge('start', 'api-status'), edge('api-status', 'end')]
  const events = [
...
function buildInvalidStartOrderFixture() {
  ...
  const edges = [edge('llm-before-start', 'start'), edge('start', 'end')]
  const events = buildEvents([
