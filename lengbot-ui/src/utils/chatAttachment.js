...
export function buildUploadHint(capabilities) {
  ...
  const docExts =
    (capabilities?.allowDocumentUpload ?? capabilities?.enableFileRead)
...
export function validateChatAttachmentFile(file, capabilities) {
  ...
  const allowDoc =
    Boolean(capabilities?.allowDocumentUpload ?? capabilities?.enableFileRead) && allowedDocExt.length > 0
  // 图片只要允许上传附件即放行：多模态直喂模型，非多模态走 OCR 兜底
  const allowImage = Boolean(capabilities?.allowFileUpload) && allowedMimes.some((m) => IMAGE_MIMES.has(m))
