<template>
  <a-modal
    ...
    centered
    :mask-closable="false"
    :body-style="{ padding: '0', height: '72vh', display: 'flex', flexDirection: 'column' }"
    @cancel="handleClose"
  >
    <!-- 编辑器区域 -->
    ...
    <div v-if="editable" class="editor-footer">
      ...
      <div class="editor-footer-right">
        ...
        <button v-if="hasChanges" class="btn-primary-sm" :disabled="saving" @click="showSaveConfirm = true">
          {{ saving ? '保存中...' : '保存并重建' }}
        </button>
        <button v-else class="btn-primary-sm" disabled>保存并重建</button>
      </div>
    </div>
  ...
  <!-- 版本历史抽屉 -->
  <a-drawer :open="showVersionDrawer" title="历史版本" :width="480" @close="showVersionDrawer = false">
