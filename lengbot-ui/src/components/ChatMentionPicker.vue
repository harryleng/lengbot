  ...
  <Teleport to="body">
    ...
    >
      <a-spin :spinning="loading" size="small">
        ...
        <template v-else>
          <div v-for="(group, gi) in visibleGroups" :key="group.type" class="mention-group">
...
function updatePos() {
  ...
  const pickerHeight = el ? Math.min(el.getBoundingClientRect().height, PICKER_MAX_HEIGHT) : 0
