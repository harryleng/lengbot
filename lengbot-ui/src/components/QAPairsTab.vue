  ...
  <div class="qa-pairs-tab">
    <div class="qa-toolbar">
      ...
      <div class="qa-toolbar-right">
        <a-button v-if="selectedRowKeys.length > 0" size="small" @click="handleBatchVectorize" :disabled="vectorizing">
          <template #icon><ApiOutlined /></template>
          向量化 ({{ selectedRowKeys.length }})
        </a-button>
            ...
            <a-tooltip :title="record.question">
              <span class="qa-cell-text">
                <a-tooltip :title="record.status">
                  ...
                  <SyncOutlined
                    v-else-if="record.status === '向量化中'"
                    class="qa-status-icon status-vectorizing"
                    spin
                  />
      ...
      <div class="dialog-scroll-body">
        <template v-if="detailRecord">
          ...
          <a-form v-if="detailEditing" :model="detailForm" layout="vertical">
            <a-form-item label="问题" required>
              <a-textarea
                v-model:value="detailForm.question"
                :rows="3"
                placeholder="输入问题内容（不超过2000字）"
                :maxlength="2000"
                show-count
            ...
            <a-form-item label="标准答案" required>
              <a-textarea
                v-model:value="detailForm.answer"
                :rows="5"
                placeholder="输入标准答案（不超过2000字）"
                :maxlength="2000"
                show-count
    ...
    >
      <a-form layout="vertical">
        ...
        <a-form-item label="生成模型">
          <ModelSelect
            v-model="generateProviderId"
            placeholder="不选择则使用系统内置模型"
