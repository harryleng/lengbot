<template>
  <div ref="wrapperRef" class="model-select-wrapper" :style="wrapperStyle">
    ...
    >
      <div class="model-select-inner" @mouseenter="selectorHovered = true" @mouseleave="selectorHovered = false">
        <a-select
          :value="displayValue"
          ...
          <template #dropdownRender="{ menuNode }">
            <div v-if="searching" class="search-progress-bar">
              <div
                ...
                :style="{ width: progress + '%', transition: progress >= 100 ? 'width 0.15s ease' : 'none' }"
              ></div>
            </div>
...
import { formatModelSelectValue, normalizeModelSelectValue, parseModelSelectValue } from '../utils/modelSelect'
...
function findOptionByLabel(labelText) {
  ...
  const list = !keyword
    ...
    : options.value.filter(
        (opt) => opt.providerName.toLowerCase().includes(keyword) || opt.modelId.toLowerCase().includes(keyword)
      )
...
const selectorTooltipOpen = computed(() => selectorHovered.value && !dropdownOpen.value && !!selectedTooltipText.value)
