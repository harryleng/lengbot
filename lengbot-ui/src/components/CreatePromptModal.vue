<template>
  <a-modal v-model:open="visible" title="快速创建 Prompt" :width="560" :mask-closable="false">
      ...
      <a-form :model="form" :label-col="{ flex: '0 0 100px' }">
        <a-form-item label="Prompt Key" required>
          <a-input
            ...
            :maxlength="30"
            show-count
            placeholder="如：my_new_prompt (不超过30字)"
            @input="sanitizePromptKey"
          />
        </a-form-item>
        <a-form-item label="描述">
          <a-textarea
            ...
            :maxlength="200"
            show-count
            placeholder="Prompt用途描述 (不超过200字)"
          />
        </a-form-item>
...
async function handleSubmit() {
  ...
  try {
    ...
    if (form.version.trim() && props.currentConfig.template) {
      const variables =
        props.currentConfig.variables?.length > 0
          ? JSON.stringify(
              props.currentConfig.variables.map((v) => ({ key: v.key, defaultValue: v.defaultValue || '' }))
            )
          : '[]'

      const modelConfig = props.currentConfig.modelConfig ? JSON.stringify(props.currentConfig.modelConfig) : '{}'
