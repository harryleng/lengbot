<template>
  <nav class="lb-breadcrumb">
    <button v-if="showArrow && items.length > 0" type="button" class="lb-breadcrumb__arrow" @click="handleArrowClick">
      <ArrowLeftOutlined />
    </button>
    <ul class="lb-breadcrumb__list">
      <li v-for="(item, idx) in items" :key="idx" class="lb-breadcrumb__item">
        <button
