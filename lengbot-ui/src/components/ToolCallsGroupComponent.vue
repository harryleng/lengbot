      ...
      <div class="tool-calls-panel">
        <div v-for="(evt, ti) in toolEvents" :key="ti" class="tool-event-item">
          ...
          <div v-if="evt.type === 'tool_call'" class="event-call-wrap">
            <div class="event-row event-call">
              ...
              <span class="event-label">
                ...
                <strong>{{ resolveDisplayName(evt) }}</strong>
                工具
              </span>
          ...
          <div v-else-if="evt.type === 'tool_result'" class="event-row event-result">
            ...
            <button
              v-if="evt.toolCallId || evt.result"
              class="result-toggle"
              @click="toggleResult(ti, $event)"
          ...
          <CollapseTransition v-if="evt.type === 'tool_result'" :open="expandedResults.has(ti)">
            <div class="result-detail">
              <ToolCallRenderer :event="evt" :args="getPairedCallArgs(ti)" :message-index="messageIndex" />
            </div>
          </CollapseTransition>
        </div>
      </div>
    </CollapseTransition>
    <a-modal v-model:open="argsModalOpen" title="参数详情" :footer="null" :width="520" destroy-on-close>
...
const uniqueToolNames = computed(() => {
  ...
  props.toolEvents.forEach((e) => {
    if (e.toolName) names.add(resolveDisplayName(e))
  })
  return [...names]
})
