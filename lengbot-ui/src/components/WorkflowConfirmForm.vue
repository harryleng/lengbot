<template>
  <div
    v-if="confirmForm"
    class="workflow-confirm-form"
    :class="{ resolved: readonly, collapsed: collapsible && !expanded }"
  >
    <button
      v-if="collapsible"
      type="button"
    ...
    >
      <span class="confirm-form-toggle-left">
        ...
        <span v-if="!expanded && !readonly" class="confirm-form-toggle-hint">
          {{ isAskUserHitl ? '点击展开回答' : '点击展开填写' }}
        </span>
      </span>
    ...
    <div v-show="!collapsible || expanded" class="confirm-form-body">
      <a-alert
        ...
        :description="
          confirmForm.message || (isAskUserHitl ? '请回答以下问题以继续工作流' : '请查看以下信息并做出选择')
        "
        class="confirm-alert"
      />
      ...
      <a-form v-else layout="vertical" class="confirm-fields">
        ...
        <a-form-item class="confirm-form-actions">
          <a-space wrap>
            ...
            <a-button v-if="showAbandon" danger :loading="abandoning" :disabled="submitting" @click="handleAbandon">
              {{ hitlAbandonLabel }}
            </a-button>
          </a-space>
          <p v-if="showAbandon" class="confirm-abandon-hint">放弃后将终止当前挂起的工作流，可重新发起对话。</p>
        </a-form-item>
      </a-form>
    </div>
  </div>
  ...
  if (isAskUserHitl.value) {
    ...
    if (!hasChoiceField) {
      ...
      if (missingLabels.length) {
        expanded.value = true
        message.warning(
          missingLabels.length === 1 ? `请填写必填项：${missingLabels[0]}` : `请填写必填项：${missingLabels.join('、')}`
        )
        return
      }
    }
