<template>
  ...
  <a-drawer v-model:open="drawerVisible" title="自动注入工具" :width="560" :body-style="{ padding: '16px' }">
    <div class="drawer-desc">这些能力由 Agent 绑定资源、个人配置或模型能力在运行时注入，不占用手动绑定工具名额。</div>
    <a-spin :spinning="loading">
      <div class="group-list">
        <section
          ...
          :key="group.key"
          class="tool-group"
          :class="{ disabled: showStatus && !group.enabled }"
        >
          <div class="group-head">
            <div>
              ...
              <p>
                ...
                <span v-if="showCount && group.triggerCount !== undefined" class="trigger-count">
                  （当前 {{ group.triggerCount }} 个）
                </span>
              </p>
            </div>
            <span v-if="showStatus" class="status-pill" :class="{ enabled: group.enabled }">
...
const groups = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      reason: subAgentEnabled
        ? '统一入口支持同步、并行与后台模式；查询和取消会回填同一个任务批次。'
        : '绑定子智能体后，将自动注入统一的批次委派能力。',
      tools: [{ name: 'delegate_to_subagent', displayName: '统一批次委派入口' }],
    },
    {
      key: 'mcp',
      title: 'MCP 工具',
      ...
      reason: mcpEnabled
        ? '具体工具由 MCP Server 运行时返回，并受 Server 工具启停配置影响。'
        : '绑定 MCP Server 后，运行时会加载该 Server 暴露的工具。',
    ...
    },
    {
      ...
      reason:
        fileReadEnabled || webSearchEnabled ? '' : '开启文件读取、联网搜索等能力后，相关系统工具会按需参与运行时链路。',
      tools: [
