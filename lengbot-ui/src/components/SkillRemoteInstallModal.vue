  ...
  >
    <div class="dialog-scroll-body">
      ...
      <div v-if="step === 'browse'">
        <a-tabs v-model:active-key="activeTab" class="remote-tabs">
          <!-- Tab 1: 按仓库拉取 -->
          <a-tab-pane key="repo" tab="按仓库拉取">
            ...
            <a-spin :spinning="fetching">
              ...
              <div v-else-if="repoFetched && !fetching" class="empty-tip">该仓库未发现包含 SKILL.md 的技能目录</div>
            </a-spin>
          </a-tab-pane>
          ...
          <a-tab-pane key="search">
            <template #tab>
              <span>
                全局搜索发现
                <QuestionCircleOutlined class="search-help-icon" @click.stop="searchGuideVisible = true" />
              </span>
            </template>
            <div class="fetch-row">
              <a-input
            ...
            <a-spin :spinning="searching">
              ...
              <div v-else-if="searchDone && !searching" class="empty-tip">未搜索到相关 Skill</div>
            </a-spin>
          </a-tab-pane>
        </a-tabs>
        ...
        <div class="browse-footer">
          ...
          <button class="btn-primary-sm" :disabled="currentSelected.length === 0" @click="handlePrepare">
        ...
        <a-spin :spinning="preparing">
          <div v-if="previews.length > 0">
            ...
            <div
              ...
              :key="(preview.slug || 'skill') + '-' + idx"
              class="preview-card"
              :class="{
                'preview-installed': committing && idx < commitProgress,
                'preview-installing': committing && idx === commitProgress,
              }"
            >
              <div class="preview-header">
                ...
                <span v-else-if="committing && idx === commitProgress" class="preview-status-tag installing">
                  <a-spin size="small" />
                  安装中
                </span>
          ...
          <div v-else-if="!preparing" class="empty-tip">未获取到 Skill 预览信息</div>
        </a-spin>

        <div class="browse-footer">
          <button class="btn-cancel" @click="step = 'browse'" :disabled="committing">返回</button>
          <button class="btn-primary-sm" :disabled="previews.length === 0 || committing" @click="handleCommit">
      ...
      <a-modal v-model:open="searchGuideVisible" title="全局搜索安装说明" :width="560" :footer="null">
        <div class="guide scroll-area-y">
          ...
          <div class="guide-section">
            ...
            <p>
              系统调用
              <code>npx -y skills find &lt;keyword&gt;</code>
              命令检索 skills.sh 社区维护的 Skill 索引，返回匹配的 Skill 列表及其来源仓库和安装量。整个过程
              <strong>无需配置 GitHub Token</strong>
              ，也不依赖 GitHub API。
            </p>
          </div>
          <div class="guide-section">
            ...
            <div class="guide-step">
              ...
              <div>
                <b>确认</b>
                <p>系统自动下载并解析 SKILL.md 元数据，确认后一键安装。</p>
              </div>
            </div>
          </div>
        </div>
...
const currentSource = computed(() => (activeTab.value === 'repo' ? repoSource.value.trim() : ''))
