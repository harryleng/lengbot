<template>
  <a-select
    ...
    style="width: 100%"
  >
    <a-select-option v-for="a in filteredAgents" :key="a.id" :value="a.id" :label="a.name">
      <EntitySelectOption :type="resolveAgentBindingType(a.agentType)" :name="a.name" :avatar-url="a.avatar" />
    </a-select-option>
  </a-select>
</template>
