<template>
  ...
  <!-- JSONL 格式说明弹窗 -->
  <a-modal v-model:open="jsonlHelpVisible" title="JSONL 格式说明" :footer="null" width="560px">
    <div class="jsonl-help">
      <p>
        <strong>JSONL（JSON Lines）</strong>
        是一种每行一个独立 JSON 对象的文本格式，用于评估基准数据的批量导入。
      </p>
      ...
      <ul>
        ...
        <li>
          <code>gold_answer</code>
          — 标准答案（可选，用于 LLM 评判）
        </li>
        <li>
          <code>gold_chunk_ids</code>
          — 标准片段 ID 数组（可选，用于检索指标计算）
        </li>
      </ul>
      <p><strong>示例文件内容：</strong></p>
      <pre class="jsonl-example">
{"query":"什么是知识图谱？","gold_answer":"知识图谱是一种结构化的语义知识库","gold_chunk_ids":["101","102"]}
...
{"query":"向量检索的优势有哪些？","gold_answer":"向量检索支持语义相似度匹配，能够理解同义词和近义表达"}</pre>
      ...
      <p style="color: #faad14; font-size: 12px; margin-top: 8px">
        <ExclamationCircleOutlined />
        文件超过 2MB 时将自动提交到任务中心异步处理，处理进度可在任务列表中查看。
      </p>
    </div>
  </a-modal>
</template>
