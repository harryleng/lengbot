<template>
  <a-modal
    ...
    width="1300px"
    :footer="null"
    :body-style="{ maxHeight: '70vh', overflowY: 'auto', padding: '16px 24px 16px 16px' }"
  ...
  >
          ...
          >
            <template #bodyCell="{ column, text, record }">
              <template v-if="column.key === 'query'">
                <a-tooltip placement="topLeft" :overlay-style="{ maxWidth: '480px' }">
              ...
              <template v-if="column.key === 'answerScore'">
                <span
                  v-if="record.answerScore != null"
                  :style="{ color: record.answerScore >= 1 ? '#52c41a' : '#ff4d4f', fontWeight: 500 }"
                >
    ...
    <a-modal
      ...
      :footer="null"
      width="680px"
      :body-style="{ maxHeight: '60vh', overflowY: 'auto', padding: '16px 24px 16px 16px' }"
    >
      <div class="metric-help-content">
        ...
        <div class="metric-help-item">
          ...
          <div class="score-rubric">
            <div class="score-rubric-row">
              <span class="score-range">1.0</span>
              <span class="score-desc">核心事实完全正确，与标准答案一致</span>
            </div>
            <div class="score-rubric-row">
              <span class="score-range">0.8 ~ 0.9</span>
              <span class="score-desc">核心事实正确，有少量非关键信息遗漏或多余</span>
            </div>
            <div class="score-rubric-row">
              <span class="score-range">0.5 ~ 0.7</span>
              <span class="score-desc">部分核心事实正确，但有明显遗漏或部分错误</span>
            </div>
            <div class="score-rubric-row">
              <span class="score-range">0.2 ~ 0.4</span>
              <span class="score-desc">仅少部分事实正确，大部分关键信息缺失或错误</span>
            </div>
            <div class="score-rubric-row">
              <span class="score-range">0.0</span>
              <span class="score-desc">完全错误、无关，或 AI 回答"无法回答"</span>
            </div>
          </div>
...
const retrievalMetrics = computed(() => {
  ...
  try {
    return JSON.parse(props.result.retrievalJson)
  } catch {
    return null
  }
})
...
const answerMetrics = computed(() => {
  ...
  try {
    return JSON.parse(props.result.answerJson)
  } catch {
    return null
  }
})
