<template>
  <div>
      ...
      <a-form :model="createForm" :label-col="{ span: labelSpan }">
        <a-form-item label="实验名称" required>
          <a-input
            ...
            :maxlength="30"
            show-count
            placeholder="如：客服 Prompt v1 vs v2 对比"
          />
        </a-form-item>
    ...
    <div v-show="currentStep === 1">
      <a-form :model="createForm" :label-col="{ span: labelSpan }">
        ...
        <a-form-item label="数据版本" required>
          <a-select v-model:value="createForm.datasetVersion" placeholder="选择数据版本" style="width: 100%">
    ...
    <div v-show="currentStep === 2">
      <a-form :model="createForm" :label-col="{ span: labelSpan }">
        ...
        <a-form-item label="Prompt 版本" required>
          <a-select v-model:value="createForm.promptVersion" placeholder="选择版本" style="width: 100%">
    ...
    <div v-show="currentStep === 3">
      <div
        v-for="(ev, idx) in createForm.evaluators"
      ...
      <a-button
        v-if="createForm.evaluators.length < 5"
        type="dashed"
        size="small"
        block
        @click="addEvaluator"
...
async function onEvaluatorChange(idx, id) {
  ...
  try {
    ...
    createForm.evaluators[idx].versions = []
  }
}
...
function nextStep() {
  ...
  if (currentStep.value === 1 && (!createForm.datasetId || !createForm.datasetVersion))
    return message.warning('请选择评测集和版本')
  if (currentStep.value === 2 && (!createForm.promptKey || !createForm.promptVersion))
