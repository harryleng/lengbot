<template>
  <div class="eval-benchmarks">
    <div class="eval-desc">管理 RAG 评估的基准数据集，支持 AI 自动生成或上传 JSONL 格式文件</div>
    ...
    <div v-if="loading && benchmarks.length === 0" class="benchmarks-empty">
      <LoadingOutlined spin class="benchmarks-loading-icon" />
      加载中...
    </div>
    ...
    <div v-else class="benchmark-list">
      <div
        v-for="bm in pagedBenchmarks"
        :key="bm.id"
        class="benchmark-card"
        :class="{ 'benchmark-generating': bm.status === 'generating' }"
      >
    ...
    >
      <div class="dialog-scroll-body">
        <div class="preview-summary">
          <span>
            问题数:
            <strong>{{ previewBenchmark_?.questionCount }}</strong>
          </span>
          <span>
            Gold Chunks:
            <strong>{{ hasGoldChunks ? '有' : '无' }}</strong>
          </span>
          <span>
            Gold Answer:
            <strong>{{ hasGoldAnswer ? '有' : '无' }}</strong>
          </span>
        </div>
        <a-table
        ...
        >
          <template #bodyCell="{ column, text }">
            ...
            <template v-if="column.key === 'goldAnswer'">
              <a-tooltip v-if="text" placement="topLeft" :overlay-style="{ maxWidth: '480px' }">
...
import { listBenchmarks, getBenchmarkDetail, deleteBenchmark, downloadBenchmark } from '../../api/knowledgeEval'
...
const previewColumns = computed(() => {
  const cols = [{ title: '问题', dataIndex: 'query', key: 'query', width: 280, ellipsis: true }]
