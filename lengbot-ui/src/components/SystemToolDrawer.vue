<template>
  ...
  <!-- 知识库工具抽屉 -->
  <a-drawer v-model:open="drawerVisible" title="知识库工具" :width="480" :body-style="{ padding: '16px' }">
    <div class="system-tool-header">
      <span class="system-tool-desc">知识库工具在 Agent 绑定知识库后自动注入，无需手动绑定。</span>
      <a-tooltip title="如何新增知识库工具？" :get-popup-container="getPopupContainer" placement="bottomRight">
        <button class="btn-help" @click="helpVisible = true"><QuestionCircleOutlined /></button>
      </a-tooltip>
    </div>
    <a-spin :spinning="loading">
      <div v-if="list.length === 0 && !loading" class="empty-tip">暂无知识库工具</div>
      <div v-for="t in list" :key="t.id" class="system-tool-item">
        ...
        <div class="tool-card-actions">
          ...
          <a-tooltip title="测试工具" :get-popup-container="getPopupContainer">
      ...
      <div class="tool-detail-modal">
        ...
        <template v-if="!rawMode">
          ...
          <div v-if="hasOutputExample" class="detail-section">
            ...
            <pre v-if="formattedOutputExample" class="example-json output-example-json">{{
  ...
  >
    <div class="dialog-scroll-body">
      ...
      <textarea v-model="testArgs" class="test-json-input" rows="6" spellcheck="false" placeholder='{"key": "value"}' />
      <LbDialogFooter
        cancel-text="关闭"
        confirm-text="执行测试"
        loading-text="执行中..."
  ...
  <!-- 知识库工具帮助弹窗 -->
  <a-modal v-model:open="helpVisible" title="如何新增知识库工具/内置工具" :width="680" :footer="null">
    <div class="dialog-scroll-body">
      <div class="help-content">
        <div class="help-section">
          <h4>什么是知识库工具？</h4>
          <p>
            知识库工具是核心内置工具，自动注入到所有 Agent，用户无需在前端绑定。例如：
            <code>query_knowledge</code>
            （知识库检索工具）。
          </p>
          ...
          <div class="help-steps">
            <div class="help-step">
              ...
              <div>
                ...
                <p>
                  在类上添加
                  <code>@SystemTool</code>
                  注解（类级别配置）：
                </p>
                <pre class="code-block">
@SystemTool(displayName = "工具名称", autoInject = false)
@Component
public class YourTool { ... }</pre>
            ...
            <div class="help-step">
              ...
              <div>
                ...
                <pre class="code-block">
@SystemTool(displayName = "方法级名称")  // 可选，覆盖类级别
    ...
    public String describeTable(...) { ... }
        ...
        <div class="help-section">
          ...
          <table class="help-table">
            ...
            <tbody>
              <tr>
                <td><code>@SystemTool</code></td>
                <td>类/方法</td>
                <td>标记为内置工具，设置 displayName、autoInject</td>
              </tr>
              <tr>
                <td><code>@Tool</code></td>
                <td>方法</td>
                <td>定义工具名称和描述（Spring AI 注解）</td>
              </tr>
              <tr>
                <td><code>@ToolParam</code></td>
                <td>参数</td>
                <td>定义参数描述（Spring AI 注解）</td>
              </tr>
              <tr>
                <td><code>@ToolParamMeta</code></td>
                <td>参数</td>
                <td>设置示例值、是否必填</td>
              </tr>
            </tbody>
        ...
        <div class="help-section">
          ...
          <ul class="help-list">
            <li>
              方法级别
              <code>@SystemTool.displayName</code>
              优先
            </li>
            <li>
              若方法无注解，使用类级别
...
const parsedSchema = computed(() => {
  ...
  try {
    const schema =
      typeof currentTool.value.inputSchema === 'string'
...
const hasOutputExample = computed(() => {
  return (
    (currentTool.value?.outputExample && currentTool.value.outputExample !== '{}') ||
    (currentTool.value?.outputSchema && currentTool.value.outputSchema !== '{}')
  )
})
...
const outputSchemaFields = computed(() => {
  ...
  try {
    const schema =
      typeof currentTool.value.outputSchema === 'string'
        ? JSON.parse(currentTool.value.outputSchema)
        : currentTool.value.outputSchema
    const properties = schema.properties || {}
    return Object.entries(properties).map(([name, prop]) => ({
      name,
      type: prop.type || 'string',
...
const testToolParams = computed(() => {
  ...
  try {
    const schema =
      typeof testTool.value.inputSchema === 'string'
