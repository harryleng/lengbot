      ...
      <template #title="node">
        <div class="sft-node" :class="{ leaf: node.isLeaf }">
          <span class="sft-node-name">
            <FileTypeIcon :name="nodeIconName(node)" :is-dir="!node.isLeaf" :size="16" class="sft-node-icon" />
            <a-tooltip
              v-if="node.isLeaf"
            ...
            >
              <span class="sft-node-label sft-node-label-clickable" @click.stop="previewFile(node.dataRef)">
                {{ node.title }}
              </span>
            </a-tooltip>
            <a-tooltip v-else :title="node.title" placement="top" :get-popup-container="tooltipPopupContainer">
...
import { LoadingOutlined, FileTextOutlined, EyeOutlined, DownloadOutlined, DeleteOutlined } from '@ant-design/icons-vue'
...
.sft-tree :deep(.ant-tree-node-content-wrapper) {
  padding-right: 4px;
}
.sft-node {
  display: flex;
...
.sft-node-label-clickable {
  cursor: pointer;
}
.sft-node-label-clickable:hover {
...
.sft-node-actions :deep(.ant-tooltip-open) {
  display: inline-flex;
}
.sft-node:hover .sft-node-actions {
  display: inline-flex;
}
.sft-act {
  width: 22px;
...
.sft-act-danger:hover {
  color: #ef4444;
  background: #fef2f2;
}
[data-theme='dark'] .sft-act-danger:hover {
