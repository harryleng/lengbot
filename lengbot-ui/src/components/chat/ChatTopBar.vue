  ...
  <div class="chat-topbar">
    <div class="chat-topbar-left">
      ...
      <div v-else class="chat-topbar-title-edit">
        ...
        />
        <a-tooltip title="取消">
          <button class="btn-title-cancel" @mousedown.prevent @click="$emit('cancel-title-edit')">
            <CloseOutlined />
          </button>
        </a-tooltip>
      </div>
    ...
    <div class="chat-topbar-right">
      <a-tooltip v-if="showRuntimePanel" :title="runtimePanelOpen ? '关闭协作状态' : '打开协作状态'">
        <button
          class="btn-topbar-file"
          :class="{ 'is-active': runtimePanelOpen }"
          @click="$emit('toggle-runtime-panel')"
        >
          <DashboardOutlined />
        </button>
      </a-tooltip>
...
.btn-topbar-harness:hover {
  background: var(--color-hairline);
  color: var(--color-ink);
}

.btn-topbar-harness.is-active {
  background: var(--color-link-bg-soft);
  color: var(--color-link-deep);
  border-color: var(--color-link-soft, var(--color-hairline));
}

.btn-topbar-harness .harness-label {
  font-size: 12px;
  font-weight: 600;
