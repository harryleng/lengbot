...
import { getToolSampleEvent, getToolSelectOptions, UNREGISTERED_TOOL_NAME } from '@/utils/chat/debug/toolDebugSamples'
...
const eventBlocks = [
  {
    ...
    events: [
      ...
      },
      {
        ...
        toolDisplayName: '知识库检索',
        args: '{"query":"LengBot 模块"}',
        contentOffset: 0,
        delegationIndex: 0,
      },
      {
...
const bodyContent = ref('根据执行结果，LengBot 已完成对应能力展示。')
...
function generateSseLog() {
  ...
  if (includeMetadata.value) {
    ...
    const toolEvents = events.filter(
      (event) => !String(event.type || '').startsWith('workflow_') && event.type !== 'reasoning_content'
    )
...
.debug-sse-block-title,
.debug-sse-block-desc {
  display: block;
}
.debug-sse-block-title {
  color: var(--color-ink);
