<template>
  <a-layout class="debug-lab-root">
        ...
        <DebugSplitPane v-model="splitRatio">
          <template #editor>
            <div class="debug-editor-pane">
              ...
              <ChatDebugPromptPanel v-else-if="activeModule === 'prompt'" ref="promptRef" @parse="parsePrompt" />

              <ChatDebugEvalPanel v-else-if="activeModule === 'eval'" ref="evalRef" @parse="parseEval" />
            </div>
          </template>
    ...
    <a-layout-footer class="debug-footer">Debug Lab · 纯前端 Mock · 与生产渲染同源 · 无需登录</a-layout-footer>
  </a-layout>
</template>
