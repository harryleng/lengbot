<template>
  <div class="debug-attachment-panel">
    <a-alert type="info" class="debug-preview-info" show-icon>
      <template #message>
        ...
        <ul v-if="showPreviewList" class="debug-preview-list">
          ...
          <li>
            <strong>纯文本 / 代码 / 数据 / 配置</strong>
            ：共 {{ TEXT_PREVIEW.length }} 种：{{ TEXT_PREVIEW.join(', ') }}
          </li>
          <li>
            <strong>Office 文档</strong>
            （不支持预览，仅显示提示）：doc, docx, xls, xlsx, ppt, pptx
          </li>
        </ul>
      </template>
    </a-alert>
    ...
    <div class="debug-panel-toolbar">
      ...
      <a-select v-model:value="selectedSample" :options="sampleOptions" style="width: 180px" @change="loadSample" />
    ...
    <input ref="fileInputRef" type="file" multiple class="debug-file-input" @change="onFilesSelected" />
    ...
    <div class="debug-attachment-list">
      <div v-for="att in attachments" :key="att.id" class="debug-attachment-item">
...
function loadSample() {
  clearAttachments()
  const imageSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="640" height="360"><rect width="640" height="360" fill="#eef2ff"/><circle cx="165" cy="170" r="72" fill="#6366f1"/><rect x="285" y="115" width="250" height="110" rx="18" fill="#14b8a6"/><text x="320" y="255" font-size="32" fill="#111827">LengBot Debug</text></svg>`
  const md = '# Debug 附件样例\n\n- 支持 Markdown 预览\n- 支持本地 Blob URL\n- 不需要上传到后端\n'
  const csv = 'name,score\nLengBot,0.96\nDebug Lab,0.91\n'
  const unsupported = 'Office placeholder'
  const createImageSamples = () => [
    createSampleAttachment({
      fileName: 'debug-image.svg',
      type: 'image',
