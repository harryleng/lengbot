  ...
  <div class="open-kb-doc-result">
    ...
    <div v-if="isPlainText" class="okd-plain">
      <div class="okd-debug-info" v-if="showDebug">
        <strong>调试信息：</strong>
        <br />
        rawResult: {{ rawResult?.substring?.(0, 50) }}...
        <br />
        data: {{ data }}
        <br />
    ...
    <template v-else>
      ...
      <div class="okd-content">
        ...
        <div v-if="isTruncated" class="okd-more-hint">内容已截断，点击"查看详情"查看全部</div>
      </div>
    </template>
      ...
      <div class="dialog-scroll-body">
        <div class="okd-modal-body">
          ...
          <div class="okd-modal-tabs">
            <button class="okd-modal-tab" :class="{ active: activeTab === 'chunks' }" @click="activeTab = 'chunks'">
              <FileTextOutlined />
              <span>分块内容</span>
            </button>
            <button
          ...
          <div v-if="activeTab === 'preview'" class="okd-modal-tab-content">
            ...
            <div v-else-if="filePreview.url">
              <iframe v-if="isPreviewableIframe" :src="filePreview.url" class="okd-iframe" frameborder="0"></iframe>
              ...
              <div v-else class="okd-unsupported">
                ...
                <p style="margin: 8px 0 0; font-size: 13px; color: #a1a1aa">
                  <template v-if="filePreview.fileType">.{{ filePreview.fileType }}</template>
                  格式不支持在线预览
                </p>
...
import { hasSourceFilePreview, isIframeSourcePreviewable, isTextSourcePreviewable } from '../../utils/filePreview'
...
const isPreviewableIframe = computed(() => isIframeSourcePreviewable(filePreview.value.fileType))

const isPreviewableText = computed(() => isTextSourcePreviewable(filePreview.value.fileType))
