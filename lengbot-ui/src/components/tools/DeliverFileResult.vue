    ...
    <template v-else>
      ...
      <div v-else class="dfr-artifacts">
        <div class="dfr-summary">
          ...
          <span v-if="data.errors?.length" class="dfr-summary-warn">（{{ data.errors.length }} 个失败）</span>
        </div>

        <div class="dfr-grid">
          <div v-for="(file, i) in data.artifacts" :key="i" class="dfr-file-card" @click="openPreview(file)">
            <!-- 图片缩略图 -->
            ...
            <!-- 下载按钮 -->
            <button type="button" class="dfr-download" @click.stop="downloadArtifact(file)" title="下载">
              <DownloadOutlined />
            </button>
          </div>
        </div>
...
function getFileIcon(contentType, name) {
  ...
  if (contentType.includes('excel') || contentType.includes('spreadsheet') || name?.endsWith('.csv'))
...
.deliver-file-result {
  ...
  .dfr-header-error .dfr-header-icon {
    color: #dc2626;
  }
