...
export function sanitizeConfirmOptions(field) {
  ...
  field.options = field.options.map((opt) => getConfirmOptionText(opt).trim()).filter(Boolean)
}
...
export function validateAskUserFormFields(formFields, formValues) {
  const fields = Array.isArray(formFields) ? formFields : []
  const hasChoice = fields.some((f) => f?.key === ASK_USER_SELECTED_OPTION_KEY && f?.type === 'radio')
