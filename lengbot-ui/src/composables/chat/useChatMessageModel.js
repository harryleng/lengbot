...
import {
  resolveWorkflowConfirmPending,
  resolveWorkflowFailureFromEvents,
  stripWorkflowErrorContent,
} from '../../components/workflow/workflowStepUtils.js'
import { safeJsonParse } from '../../utils/request'
import {
  contentHasMentionTokens,
  parseMentionsFromMetadata,
  resolveMentionsForDisplay,
...
export function getMsgMentions(msg) {
  ...
  const raw =
    Array.isArray(msg._mentions) && msg._mentions.length ? msg._mentions : parseMentionsFromMetadata(msg.metadata)
...
export function parseMessage(m) {
  ...
  const workflowFailure =
    resolveWorkflowFailureFromEvents(workflowEvents) ||
