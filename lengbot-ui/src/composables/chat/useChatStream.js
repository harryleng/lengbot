...
export function useChatStream(deps) {
  ...
  async function runChatStream({
    ...
    try {
        ...
        chatPayload,
        {
          ...
          onToolEvent: (event) => {
            ...
            if (event.type === 'error_retry') {
              // 中途断流重试（resetContent=true）：后端已回滚 fullReply，前端须清空已展示的
              // 部分正文/思考/工具块，让重传从头追加，否则重跑内容会接在旧内容后面。
              if (event.resetContent && assistantMsg) {
                assistantMsg.content = ''
                assistantMsg._reasoningContent = ''
                assistantMsg._reasoningDone = false
                assistantMsg._toolEvents = []
                assistantMsg._workflowEvents = []
                assistantMsg._toolBlockOffsets = []
                assistantMsg._toolBlocksDone = []
                assistantMsg._currentToolOffset = 0
            ...
            const isWorkflowStreamEvent =
              event.type === 'workflow_llm_chunk' ||
      ...
      // 401：登录过期（Sa-Token 24h 活跃窗口）— 单独提示，不再套用「模型调用失败」文案
      if (e.status === 401) {
        assistantMsg._error = {
          message: '登录已过期，请退出后重新登录再试。\n\n错误详情：' + (e.message || '未登录或登录已过期'),
          code: 'AUTH_EXPIRED',
        }
      } else {
        assistantMsg._error = {
  ...
  function stopGenerating() {
    ...
    stopFallbackTimer = setTimeout(() => {
      ...
      const msg =
        getCurrentStreamingMsg() || [...messages.value].reverse().find((m) => m.role === 'assistant' && m._streaming)
