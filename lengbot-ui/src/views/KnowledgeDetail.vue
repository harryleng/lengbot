<template>
  <div v-if="knowledge && docPagination && typeof askRag === 'function'" class="detail-page">
    ...
    >
      ...
      <template #stats>
        <a-tooltip :title="statsRepairing ? '统计中...' : '重新统计'">
          <button type="button" class="kb-stats-group" :disabled="statsRepairing" @click="handleRefreshStats">
      ...
      <main class="kb-tab-content">
        ...
        <div v-show="activeTab === 'docs'" class="kb-tab-pane">
          <div class="doc-toolbar">
            ...
            <a-tooltip
              title="刷新"
              :open="docRefreshTipVisible"
              @open-change="
                (v) => {
                  if (!v) docRefreshTipVisible = false
                }
              "
            >
            ...
            <template #bodyCell="{ column, record }">
                ...
                <div class="doc-name-cell">
                  ...
                  <a-tag
                    v-if="
                      ...
                      record.duplicateRate != null &&
                      record.duplicateRate >= duplicateThreshold
                    "
                    color="orange"
                    class="doc-dup-tag"
                  >
                    <a-tooltip
                      :title="`内容重复率 ${(record.duplicateRate * 100).toFixed(1)}%，超过阈值 ${(duplicateThreshold * 100).toFixed(0)}%`"
                    >
                ...
                <div class="doc-status-cell">
                  <a-tooltip :title="statusText(record.status?.code || record.status)">
                    <span class="doc-status-icon" :class="record.status?.code || record.status">
                      ...
                      <SyncOutlined
                        v-else-if="
                          (record.status?.code || record.status) === 'uploading' ||
                          (record.status?.code || record.status) === 'pending' ||
                          (record.status?.code || record.status) === 'processing'
                        "
                        spin
                      />
              ...
              <template v-else-if="column.key === 'action'">
                <div class="doc-action-cell" @click.stop>
                  <a-tooltip
                    v-if="
                      (record.status?.code || record.status) === 'uploaded' ||
                      (record.status?.code || record.status) === 'failed'
                    "
                    title="入库"
                  >
          ...
          <div class="rag-section">
            <div class="rag-messages" ref="ragRef">
              <template v-for="(turn, ti) in ragHistory" :key="ti">
                ...
                <template v-else>
                  ...
                  <div v-else class="rag-msg assistant">未检索到相关内容</div>
            ...
            <div v-else-if="ragHistory.length === 0" class="example-questions">
              <div class="example-questions-hint">
                暂无示例问题，
                <a @click="handleGenerateQuestions" :disabled="editQuestionLoading">点击生成示例问题</a>
              </div>
            </div>
          </div>
        </div>
    ...
    <a-modal
      ...
      :footer="null"
      centered
      :body-style="{ padding: '0', height: 'calc(100vh - 160px)', overflow: 'hidden' }"
    >
      <a-tabs v-model:active-key="docModalTab" class="doc-modal-tabs">
        <template #rightExtra>
          <div class="doc-modal-actions">
            <a-tooltip
              v-if="
                isEditableType(currentDoc?.fileType) && (currentDoc?.status?.code || currentDoc?.status) === 'completed'
              "
              title="编辑文档"
            >
          ...
          <div class="tab-pane-body doc-info-pane">
            <div class="doc-info-section">
              ...
              <a-descriptions
                ...
                size="small"
                bordered
                :label-style="{ width: '100px', minWidth: '100px', whiteSpace: 'nowrap' }"
              >
                ...
                <a-descriptions-item label="状态">
                  <a-tag :color="docStatusColor(currentDoc?.status)">
                    {{ statusText(currentDoc?.status?.code || currentDoc?.status) }}
                  </a-tag>
                </a-descriptions-item>
            ...
            <div class="doc-info-section">
              ...
              >
                ...
                <a-descriptions-item label="Token 数量">
                  {{ currentDoc?.tokenCount != null ? currentDoc.tokenCount.toLocaleString() : '-' }}
                </a-descriptions-item>
                <a-descriptions-item label="内容重复率">
                  <template v-if="currentDoc?.duplicateRate != null">
                    <a-tag
                      :color="
                        currentDoc.duplicateRate >= 0.8 ? 'red' : currentDoc.duplicateRate >= 0.5 ? 'orange' : 'green'
                      "
                    >
                ...
                <a-descriptions-item label="入库策略">
                  {{
            ...
            <div
              v-if="currentDoc?.errorMessage && (currentDoc?.status?.code || currentDoc?.status) === 'failed'"
              class="doc-info-section"
            >
        ...
        <a-tab-pane v-if="hasSourcePreview" key="source" tab="源文件预览">
          <div class="tab-pane-body">
            <FilePreview
              ...
              :file-name="currentDoc?.name"
              :file-type="currentDoc?.fileType"
        ...
        <a-tab-pane v-if="isDocCompleted" key="knowledge-graph" tab="知识图谱">
          <div class="tab-pane-body doc-graph-pane">
            <KnowledgeGraphTab
              :key="`doc-graph-${currentDoc?.id}-${docModalKey}`"
              :knowledge-id="knowledgeId"
    ...
    <!-- 上传文档弹窗 -->
    <a-modal v-model:open="uploadVisible" :width="uploadMode === 'url' ? 640 : 520" :footer="null">
      <template #title>
        <span>上传文档</span>
        <a-tooltip placement="right" :overlay-style="{ maxWidth: '400px', fontSize: '12px' }">
          ...
          <template #title>
            <div class="supported-file-list">
              ...
              <div>
                <strong>配置类：</strong>
                ini, conf, cfg, toml, properties, env, editorconfig, gradle
              </div>
              <div>
                <strong>代码类：</strong>
                js, mjs, cjs, ts, tsx, jsx, vue, py, java, go, rs, rb, php, c, h, cpp, hpp, cc, cs, kt, kts, scala,
                swift, sql, sh, bash, zsh
              </div>
              <div style="margin-top: 4px; color: var(--color-mute)">
                Office 文档（doc/docx/xls/xlsx/ppt/pptx）仅支持内容解析，不支持源文件预览
              </div>
            </div>
          </template>
        </a-tooltip>
      ...
      <div class="upload-section">
        ...
        <div class="upload-mode-switch">
          <a-segmented v-model:value="uploadMode" :options="uploadModeOptions" size="small" />
        </div>
        ...
        >
          ...
          <p class="dropzone-hint">
            支持 md/txt/pdf/doc/docx/ppt/pptx/xls/xlsx/csv/html/json/yaml 等，单文件最大 100MB
          </p>
        </div>
          ...
          <div v-for="(item, i) in urlList" :key="(item.url || 'url') + '-' + i" class="url-item">
            ...
            <div class="url-main">
              <div class="url-row-top">
                <a class="url-link" :href="item.url" target="_blank" rel="noopener noreferrer" :title="item.url">
                  {{ item.url }}
                </a>
              ...
              <div v-if="item.status === 'success'" class="url-row-bottom">
                ...
                <span v-if="item.contentLength" class="url-char-badge">
                  {{ formatUrlContentLength(item.contentLength) }}
                </span>
              </div>
        ...
        <div v-if="uploadMode === 'url' && urlList.length === 0" class="url-empty-tip">
          <GlobalOutlined />
          输入 URL 后点击解析，解析成功后可预览网页内容，确认无误后再添加
        </div>
        ...
        <div class="upload-actions">
          ...
          <button
            v-if="uploadMode === 'url'"
            class="btn-primary-sm"
            :disabled="!hasSuccessfulUrls || urlSaving"
            @click="handleConfirmUrls"
          >
            {{ urlSaving ? '添加中...' : '确认添加' }}
          </button>
        </div>
    ...
    <!-- 重复文档检测弹窗 -->
    <a-modal v-model:open="dupDetectVisible" title="检测到重复文档" :width="460" :footer="null" :mask-closable="false">
      ...
      <div class="dup-detect-actions">
        <button class="btn-outline" @click="handleDupChoice('skip')">跳过，不上传</button>
        <button class="btn-outline" @click="handleDupChoice('keep-both')">保留两个版本</button>
        <button class="btn-primary-sm" @click="handleDupChoice('overwrite')">覆盖旧文档</button>
      </div>
    </a-modal>
    ...
    >
      <div v-if="urlPreviewItem" class="url-preview-modal">
        ...
        <a-tabs v-model:active-key="urlPreviewTab">
          <a-tab-pane key="html" tab="网页预览">
            <div v-if="urlPreviewItem.previewHtml" class="url-preview-html" v-html="urlPreviewItem.previewHtml" />
        ...
        <a-form :model="ingestForm" :label-col="{ flex: '0 0 100px' }">
          <a-form-item label="分块策略" required>
            ...
            <div
              ...
              class="default-strategy-hint"
            >
              <a-tag color="blue">
                使用知识库配置: {{ strategyLabelMap[knowledgeDefaultStrategy] || knowledgeDefaultStrategy }}
              </a-tag>
            </div>
          </a-form-item>
          <a-form-item label="分块大小" required>
            <div style="display: flex; align-items: center; gap: 6px">
              <a-input-number
                v-model:value="ingestForm.chunkSize"
                :min="100"
                :max="2000"
                :step="100"
                style="width: 100%"
              />
              <a-tooltip
                title="每个分块的最大Token数。过小的分块（<30 tokens）会被自动过滤，建议值 200-1000。设置过小可能导致所有分片被过滤，入库失败。"
              >
        ...
        <a-form :model="editForm" :label-col="{ flex: '0 0 100px' }">
          ...
          <a-form-item label="知识图谱">
            <div style="display: flex; align-items: center; gap: 8px">
              ...
              <a-tooltip
                title="开启后，文档入库完成时自动触发知识图谱抽取，提取实体和关系用于图谱检索增强（RAG Graph）"
              >
                <QuestionCircleOutlined style="font-size: 14px; color: var(--color-mute); cursor: help" />
              </a-tooltip>
            </div>
          ...
          <a-form-item v-if="editForm.duplicateDetectionEnabled" label="重复度阈值">
            <div style="display: flex; align-items: center; gap: 8px">
              ...
              />
              <a-tooltip
                title="文档内容重复度阈值，超过此值的文档会被标记为重复警告（用于检测文档间重复，非检索相似度阈值）。建议值 0.7-0.9"
              >
                <QuestionCircleOutlined style="font-size: 14px; color: var(--color-mute); cursor: help" />
              </a-tooltip>
            </div>
            ...
            <div class="edit-questions-list">
              ...
              <div class="edit-question-actions">
                <a-button
                  ...
                  :disabled="editExampleQuestions.length >= 10"
                >
                  <PlusOutlined />
                  新增
                </a-button>
                ...
                <a-tooltip title="知识库的示例问题会展示在RAG问答界面，帮助用户快速了解知识库内容。最多10个。">
                  <QuestionCircleOutlined
                    style="font-size: 14px; color: var(--color-mute); cursor: help; margin-left: 4px"
                  />
                </a-tooltip>
              </div>
            </div>
          ...
          <div v-for="member in membersWithInfo" :key="member.userId" class="member-item">
            ...
            <div class="member-actions" v-if="isManagerOrCreator">
              ...
              <a-tooltip v-if="member.role !== 'creator'" title="移除成员">
                <button class="btn-icon-sm danger" @click="handleRemoveMember(member.userId)">
                  <CloseOutlined />
                </button>
              </a-tooltip>
            </div>
    ...
    <!-- 邀请成员弹窗 -->
    <a-modal v-model:open="inviteVisible" title="邀请成员" :width="480" :footer="null" :mask-closable="false">
      <div class="invite-section">
        <a-input v-model:value="inviteKeyword" placeholder="搜索用户名或昵称..." allow-clear @input="onInviteSearch">
          <template #prefix><SearchOutlined /></template>
        </a-input>
        <div class="invite-results">
          ...
          <template v-else>
            <div v-for="u in inviteResults" :key="u.id" class="invite-item">
              ...
              <a-select v-model:value="inviteRole" size="small" style="width: 90px">
                <a-select-option value="manager">管理者</a-select-option>
                <a-select-option value="developer">开发者</a-select-option>
                <a-select-option value="viewer">查看者</a-select-option>
              </a-select>
    ...
    <!-- 文档编辑弹窗 -->
    <DocumentEditorModal v-model:open="docEditorVisible" :document-id="docEditorDocId" @saved="onDocEditorSaved" />
...
import {
  ...
  removeKnowledgeMember,
  ingestDocument,
  previewChunks,
  getDefaultIngestConfig,
  checkOcrHealth,
  generateExampleQuestions,
  getExampleQuestions,
  updateExampleQuestions,
  generateOneExampleQuestion,
  fetchUrlDocument,
  previewUrlDocument,
  saveUrlDocument,
  getQueryParams,
...
const DocumentEditorModal = defineAsyncComponent(() => import('../components/DocumentEditor/DocumentEditorModal.vue'))
...
const knowledgeDefaultStrategy = computed(() => {
  try {
    const cfg =
      typeof knowledge.value.config === 'string' ? JSON.parse(knowledge.value.config) : knowledge.value.config || {}
    ...
    Modal.confirm({
      ...
      content: h('div', [
        h(
          ...
          {
            style:
              'max-height:200px;overflow-y:auto;background:#fafafa;border-radius:6px;padding:8px 12px;margin-bottom:12px',
          },
          pendingItems.map((item) => h('div', { style: 'word-break:break-all;padding:2px 0;font-size:13px' }, item.url))
        ),
...
async function handleConfirmUrls() {
  ...
  if (validHeaders.length > 0) {
    ...
    validHeaders.forEach((h) => {
      syncConfig.headers[h.key.trim()] = h.value || ''
    })
  }
...
const hasSourcePreview = computed(() => hasSourceFilePreview(currentDoc.value?.fileType || currentDoc.value?.name))
...
[data-theme='dark'] .kb-tab-item.active {
  color: var(--color-link);
}
...
.doc-status-icon.processing {
  color: #d97706;
}
.doc-status-icon.completed {
  color: #16a34a;
}
.doc-status-icon.failed {
  color: #dc2626;
}
/* 操作列按钮组 */
.doc-action-cell {
