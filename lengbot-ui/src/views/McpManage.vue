  ...
  <div class="page">
    ...
    <a-spin :spinning="loading" style="min-height: 300px; display: block">
      <div class="provider-grid">
        <EntityCard v-for="s in list" :key="s.id" type="mcp" :name="s.name" @click="openDetail(s)">
          <template #icon>
    ...
    <!-- 新增/编辑弹窗 -->
    <a-modal v-model:open="dialogVisible" :width="560" :mask-closable="false">
      <template #title>
        ...
        <a-form :model="form" :label-col="{ flex: '0 0 100px' }">
          <a-form-item label="名称" required>
            <a-input
              ...
              placeholder="如：filesystem-server（不超过30字）"
              :maxlength="30"
              show-count
            />
          </a-form-item>
          ...
          <template v-if="form.installType === 'npx' || form.installType === 'uvx'">
            <a-form-item label="包名">
              <a-input
                v-model:value="deployForm.packageName"
                placeholder="如：@modelcontextprotocol/server-filesystem"
              />
            </a-form-item>
            <a-form-item label="命令参数">
              <a-textarea
                v-model:value="deployForm.args"
                :rows="2"
                placeholder="每行一个参数，如：--allow-read&#10;/tmp"
              />
            </a-form-item>
      ...
      <div class="dialog-scroll-body">
        <div class="guide">
          <div class="guide-section">
            ...
            <p>
              MCP (Model Context Protocol) 是 Anthropic 开放的 AI 工具协议，让 AI
              能调用外部工具（如文件操作、图表生成、数据库查询等）。
            </p>
          </div>
...
/tmp</pre>
          ...
          <div class="guide-section">
            ...
            <div class="guide-code">
              ...
              <div>
                <b>传输协议</b>
                ：SSE 或 Streamable HTTP（根据服务端支持选择）
              </div>
              <div>
                <b>服务地址</b>
          ...
          <div class="guide-section">
            ...
            <p>
              部分 MCP Server 需要 API Key 等环境变量，格式为
              <code>KEY=VALUE</code>
              ，每行一个。例如：
            </p>
            <pre class="guide-pre">
...
OPENAI_API_KEY=sk-xxxxxxxxxxxx</pre>
    ...
    <!-- 详情弹窗 -->
    <a-modal v-model:open="detailVisible" title="MCP Server 详情" :width="640" :mask-closable="false">
      <div class="dialog-scroll-body">
          ...
          <template v-if="detailRow">
            <a-descriptions bordered :column="1" size="small">
              ...
              <a-descriptions-item label="传输协议">
                {{ detailRow.transport?.code || detailRow.transport || 'stdio' }}
              </a-descriptions-item>
        ...
        <LbDialogFooter cancel-text="关闭" hide-confirm @cancel="detailVisible = false">
          <template #left>
            <button
              v-if="detailRow"
              class="lb-btn"
              @click="() => { detailVisible = false; openDialog(detailRow) }"
            >
              <EditOutlined />
              编辑
            </button>
    ...
    <!-- 工具列表抽屉 -->
    <a-drawer v-model:open="toolsDrawerVisible" title="MCP 工具列表" :width="520" :body-style="{ padding: '16px' }">
      <div class="tools-header">
        ...
        <button class="btn-refresh" :disabled="toolsLoading" @click="handleRefreshTools">
          <SyncOutlined :spin="toolsLoading" />
          刷新工具
        </button>
      </div>
      <a-spin :spinning="toolsLoading">
        ...
        <div v-else-if="toolsList.length === 0 && !toolsLoading" class="tools-empty">
          暂无工具，请检查 MCP Server 配置
        </div>
    ...
    <!-- 工具详情弹窗 -->
    <a-modal v-model:open="toolDetailVisible" :title="toolDetail?.name || '工具详情'" :width="720" :footer="null">
      <div class="dialog-scroll-body">
    ...
    >
      ...
      <template #footer>
        <LbDialogFooter hide-cancel confirm-text="知道了" @confirm="testResultVisible = false" />
      </template>
    </a-modal>
  </div>
</template>
...
const parsedSchema = computed(() => {
  ...
  try {
    const schema =
      typeof toolDetail.value.inputSchema === 'string'
