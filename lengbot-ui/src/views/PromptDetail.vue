<template>
  <div class="detail-page">
          ...
          <div v-for="(inst, idx) in instances" :key="inst.id" class="instance-card">
              ...
              <div class="model-params" v-if="inst.configFields.length > 0">
                <div class="param-row" v-for="field in inst.configFields" :key="field.key">
                  ...
                  <a-switch
                    ...
                    size="small"
                  />
                  <a-input v-else v-model:value="inst.modelConfig[field.key]" size="small" style="width: 120px" />
            ...
            <div class="config-section" v-if="inst.variables.length > 0">
              ...
              <div class="variable-row" v-for="v in inst.variables" :key="v.key">
                ...
                <a-input v-model:value="v.defaultValue" size="small" :placeholder="'默认值'" style="flex: 1" />
              </div>
            </div>
            ...
            <div class="debug-area">
              ...
              >
                ...
                <div
                  v-for="(msg, i) in inst.messages"
                  :key="msg.id || msg._id || i"
                  :class="['debug-msg-row', msg.role, { 'is-error': msg._error }]"
                >
                  <!-- AI 调用失败：专用报错样式 -->
                  ...
                  <template v-else>
                    <div class="msg-bubble">
                      <MarkdownPreview
                        v-if="msg.role === 'assistant' && !msg._raw"
                        :content="msg.content"
                        :finalized="true"
                      />
    ...
    <!-- 从模板导入弹窗 -->
    <TemplateImportModalLR v-model:open="templateImportVisible" @import="handleTemplateImport" />

    <!-- 发布版本弹窗 -->
    <a-modal v-model:open="versionDialogVisible" title="发布版本" :width="480" :mask-closable="false">
...
async function applyVersionToInstance(inst, versionData) {
  ...
  if (versionData.modelConfig) {
    try {
      const cfg =
        typeof versionData.modelConfig === 'string' ? JSON.parse(versionData.modelConfig) : versionData.modelConfig
...
.instances-grid.cols-1 {
  grid-template-columns: 1fr;
}
.instances-grid.cols-2 {
  grid-template-columns: repeat(2, 1fr);
}
.instances-grid.cols-3 {
