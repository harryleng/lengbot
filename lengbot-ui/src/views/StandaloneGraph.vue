<template>
  <div class="standalone-graph-page">
            ...
            <template #title>
              <div style="max-width: 260px">
                ...
                <div style="margin-top: 6px; color: #bbb">
                  示例：搜索"数据库技术"可以找到"MySQL"、"PostgreSQL"等节点
                </div>
              </div>
            </template>
      ...
      <div class="dialog-scroll-body">
        <a-form :model="edgeForm" :label-col="{ flex: '0 0 90px' }">
          <a-form-item label="起始节点">
            <a-input v-if="editingEdge" :value="edgeForm.headName" disabled />
            <a-select
              v-else
          ...
          <a-form-item label="目标节点">
            <a-input v-if="editingEdge" :value="edgeForm.tailName" disabled />
            <a-select
              v-else
    ...
    <!-- 导入 JSONL 弹窗 -->
    <a-modal v-model:open="showImportModal" title="导入知识图谱" :mask-closable="false" :footer="null" :width="560">
      <div class="dialog-scroll-body">
        ...
        <div class="import-section">
          ...
          <pre class="import-format-pre">
{"head": "张三", "headType": "人物", "headDesc": "软件工程师",
 ...
 "tail": "腾讯", "tailType": "组织", "tailDesc": "互联网公司"}</pre>
        ...
        <div class="import-actions">
          <a-upload :before-upload="handleJsonlUpload" :show-upload-list="false" accept=".jsonl">
...
import {
  getStandaloneSubgraph,
  getStandaloneStats,
  deleteStandaloneGraph,
  importGraphFromJsonl,
  semanticSearchGraph,
  createStandaloneNode,
  updateStandaloneNode,
  deleteStandaloneNode,
  createStandaloneEdge,
  updateStandaloneEdge,
  deleteStandaloneEdge,
