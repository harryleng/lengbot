...
export function getNodeLibraryGroups(search = '') {
  ...
  return NODE_LIBRARY_GROUPS.map((group) => ({
    ...
    items: group.types.filter((type) => {
      ...
      if (!kw) return true
      return meta.title.toLowerCase().includes(kw) || (meta.desc || '').toLowerCase().includes(kw) || type.includes(kw)
    }),
...
export function getDefaultNodeData(type) {
  const defaults = {
    ...
    tool: {
      ...
      toolId: null,
      toolName: '',
      inputMappings: [{ key: 'query', value: '{{query}}' }],
      outputMappings: [{ key: 'toolResult', value: '{{output}}' }],
    },
    api: {
      label: 'HTTP API',
      url: '',
