...
function getEventExecutionKey(event) {
  ...
  return [event.parentNodeId || '-', event.nodeId || '-', event.iterationIndex ?? '-'].join(':')
}
...
export function useWorkflowNodeSteps(nodeEventsSource) {
  const nodeSteps = computed(() => {
    const events =
      typeof nodeEventsSource === 'function' ? nodeEventsSource() : (nodeEventsSource?.value ?? nodeEventsSource)
    ...
    for (const e of events) {
      ...
      if (e.type === 'workflow_node_start' && e.nodeId) {
        ...
        let step =
          runningByExecutionKey.get(executionKey) || (e.stepIndex != null ? stepByIndex.get(e.stepIndex) : null)
        if (!step) {
