  ...
  >
    ...
    <template v-if="drawerTab === 'current'">
      ...
      <p class="test-mode-hint">
        {{
          testMode === 'generation'
            ? '单轮生成：输入一次问题，执行完整工作流并返回结果。'
            : '多轮对话：保留历史消息，每轮携带 history_list / query 变量执行。'
        }}
      </p>
      ...
      <a-form layout="vertical">
        ...
        <div class="test-actions">
          ...
          <a-button
            v-if="testMode === 'conversation'"
            :disabled="testRunning || testStreaming || testAnimating || viewingHistory"
      ...
      <div v-if="(testStreaming || testAnimating) && testCurrentNodeId" class="test-current-node">
        {{ viewingHistory ? '回放节点' : '当前节点' }}：
        <strong>{{ getNodeTitleById(testCurrentNodeId) }}</strong>
      </div>
      <a-alert
      ...
      <div v-if="testResult">
        ...
        <pre v-if="!testPendingConfirm || viewingHistory" class="test-output">{{
        ...
        <div v-if="resultTab === 'variables' && testResult.variables" class="vars-panel">
          ...
          <div
            v-for="(val, key) in testResult.variables"
            :key="key"
            class="var-item"
            :class="{ expanded: expandedVars.has(key) }"
          >
      ...
      <div class="history-toolbar">
        ...
        <div class="history-toolbar-actions">
          ...
          <a-button
            size="small"
            danger
            :disabled="!historyList.length || historyLoading"
        ...
        <div v-else class="history-list">
          ...
          >
            <div class="history-item-head">
              ...
              <a-button type="text" size="small" danger @click.stop="$emit('delete-history', item.runId)">
                删除
              </a-button>
            </div>
