<template>
  <div class="workflow-toolbar">
    ...
    <a-tag v-else-if="workflowStatus === 'published_editing'" color="gold" class="publish-tag">
      {{ getStatusLabel('published_editing') }}
    </a-tag>
    ...
    <div class="toolbar-status">
      ...
      <WorkflowTooltip title="格式化" placement="bottom">
        <a-button
          ...
          size="small"
          class="btn-validate"
          :disabled="isVersionPreview || nodeCount < 2"
          @click="$emit('format-layout')"
        >
          <ApartmentOutlined />
        </a-button>
      </WorkflowTooltip>
      ...
      <span v-else-if="lastAutoSaveTime" class="auto-save-hint">
        {{ formatAutoSaveTime(lastAutoSaveTime) }} 已自动保存
      </span>
    </div>
