<template>
  <div :class="readonlyRootClass">
    ...
    <div
      v-if="readonly && readonlyScrollable"
      class="readonly-overlay readonly-overlay--scrollable"
      aria-hidden="true"
    />
    <a-form
      layout="vertical"
    ...
    >
      ...
      <template v-if="node.type === 'llm'">
        ...
        <a-form-item>
          ...
          <a-textarea
            v-model:value="node.data.sysPrompt"
            :rows="2"
            placeholder="定义 AI 角色、行为约束（对应 SystemMessage）"
            @change="emitSync"
          />
        </a-form-item>
        <a-form-item required>
      ...
      <template v-if="node.type === 'classifier'">
        <a-form-item required>
          ...
          <VariablePickerInput
            v-model="node.data.inputVariable"
            :node-id="node.id"
            :nodes="nodes"
            :edges="edges"
            placeholder="{{query}}"
        ...
        <a-form-item required>
          ...
          <a-button type="dashed" block size="small" class="param-add-btn" @click="addIntent">
            <PlusOutlined />
            添加意图（{{ node.data.conditions?.length || 0 }}/10）
          </a-button>
        </a-form-item>
        ...
        <a-form-item>
          ...
          <a-textarea
            v-model:value="node.data.instruction"
            :rows="3"
            placeholder="为意图识别提供额外要求"
            @change="emitSync"
          />
        </a-form-item>
      ...
      <template v-if="node.type === 'tool'">
          ...
          >
            ...
            >
              <EntitySelectOption
                ...
                :icon="t.icon"
                :tag="getToolTypeLabel(t.toolType)"
                :desc="t.description"
              />
            </a-select-option>
          </a-select>
        ...
        <a-form-item>
          ...
          <div v-for="(row, idx) in toolInputMappings" :key="'tool-in-' + idx" class="param-row">
            ...
            />
            <a-button v-if="!readonly" type="text" danger @click="removeToolInputMapping(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
          <a-button
        ...
        <a-form-item>
          ...
          <div v-for="(row, idx) in toolOutputMappings" :key="'tool-out-' + idx" class="param-row">
            ...
            <VariablePickerInput
              ...
              @update:model-value="emitSync"
            />
            <a-button v-if="!readonly" type="text" danger @click="removeToolOutputMapping(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
          <a-button
      ...
      <template v-if="node.type === 'input'">
        <div class="config-section">
          ...
          <div
            v-for="(param, idx) in node.data.outputParams"
      ...
      <template v-if="node.type === 'confirm'">
        <a-form-item>
          ...
          <a-textarea
            v-model:value="node.data.message"
            :rows="2"
            placeholder="展示给操作者的说明文字"
            @change="emitSync"
          />
        </a-form-item>
        <div class="config-section">
      ...
      <template v-if="node.type === 'output'">
        <a-form-item label="输出内容" required>
          <a-textarea
            v-model:value="node.data.output"
            :rows="4"
            placeholder="{{input}} 或 {{llmOutput}}"
            @change="emitSync"
          />
        </a-form-item>
      ...
      <template v-if="node.type === 'variable_handle'">
        ...
        <a-form-item v-if="node.data.handleType === 'template'" label="模板内容" required>
          <a-textarea
            v-model:value="node.data.templateContent"
            :rows="4"
            placeholder="支持 {{变量名}}"
            @change="emitSync"
          />
        ...
        <template v-if="node.data.handleType === 'group'">
          ...
          <a-form-item label="变量列表">
            ...
            <a-button type="dashed" block size="small" class="param-add-btn" @click="addGroupVar">
              <PlusOutlined />
              添加变量
            </a-button>
          </a-form-item>
      ...
      <template v-if="node.type === 'parameter_extractor'">
        ...
        <a-form-item required>
          ...
          <div
            v-for="(p, idx) in node.data.extractParams"
            :key="(p.key || 'extract') + '-' + idx"
            class="extract-param-row"
          >
          ...
          <a-button type="dashed" block size="small" class="param-add-btn" @click="addExtractParam">
            <PlusOutlined />
            添加参数
          </a-button>
        </a-form-item>
        ...
        <a-form-item>
          ...
          <div v-for="(row, idx) in appComponentInputMappings" :key="'sub-in-' + idx" class="param-row">
            ...
            <VariablePickerInput
              ...
              @update:model-value="emitSync"
            />
            <a-button v-if="!readonly" type="text" danger @click="removeAppInputMapping(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
        ...
        <a-form-item>
          ...
          <div v-for="(row, idx) in appComponentOutputMappings" :key="'sub-out-' + idx" class="param-row">
            ...
            <VariablePickerInput
              ...
              @update:model-value="emitSync"
            />
            <a-button v-if="!readonly" type="text" danger @click="removeAppOutputMapping(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
          <a-button
      ...
      <template v-if="node.type === 'loop'">
        ...
        <a-form-item>
          ...
          <div
            v-for="(p, idx) in node.data.output_params || node.data.outputParams || []"
            :key="'loop-out-' + idx"
            class="param-row"
          >
            ...
            <a-button type="text" danger :disabled="readonly" @click="removeLoopOutputParam(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
          <a-button type="dashed" block class="param-add-btn" :disabled="readonly" @click="addLoopOutputParam">
            + 添加输出
          </a-button>
        </a-form-item>
      </template>
      ...
      <template v-if="node.type === 'batch'">
        ...
        <a-form-item>
          ...
          >
            ...
            <a-button type="text" danger :disabled="readonly" @click="removeBatchOutputParam(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
          <a-button type="dashed" block class="param-add-btn" :disabled="readonly" @click="addBatchOutputParam">
            + 添加输出
          </a-button>
        </a-form-item>
      </template>
      ...
      <template v-if="node.type === 'script'">
        ...
        <div class="config-section">
          ...
          <div v-for="(p, idx) in node.data.inputParams" :key="'in-' + idx" class="param-row">
            ...
            <VariablePickerInput
              ...
              @change="emitSync"
            />
            <a-button type="text" danger :disabled="readonly" @click="removeScriptInput(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
        ...
        <div class="config-section">
          ...
          <div v-for="(p, idx) in node.data.outputParams" :key="'out-' + idx" class="param-row">
            ...
            <a-button type="text" danger :disabled="readonly" @click="removeScriptOutput(idx)">
              <DeleteOutlined />
            </a-button>
          </div>
          <a-button
      ...
      <template v-if="node.type === 'mcp'">
        <a-form-item label="MCP 服务" required>
          ...
          >
            <a-select-option v-for="s in mcpServers" :key="s.id" :value="s.id" :label="s.name">
              <div class="resource-option">
                <div class="resource-option-header">
                  <EntitySelectOption
                    type="mcp"
                    :name="s.name"
                    :icon="s.icon"
                    :builtin="s.isBuiltin === 1"
                    :tag="mcpInstallTypeLabel(s)"
                  />
                </div>
        ...
        <a-form-item label="工具" required>
          <div class="mcp-tool-picker">
            ...
            <a-select
              ...
              @change="emitSync"
            >
              <a-select-option v-for="t in enabledMcpTools" :key="t.name" :value="t.name" :label="t.name">
                <div class="resource-option">
                  <div class="resource-option-header">
        ...
        <a-form-item label="输入参数 JSON">
          <JsonInput
            v-model="mcpInputParamsJson"
            :rows="4"
            :readonly="scrollableReadonly"
            placeholder='{"chat_id":"oc_xxx","text":"{{query}}"}'
          />
        </a-form-item>
      </template>
...
const emit = defineEmits(['sync', 'knowledge-change', 'tool-change'])
...
async function loadPublishedWorkflowAgents() {
  ...
  try {
    ...
    publishedWorkflowAgents.value = records.filter((a) => String(a.id) !== currentId && Number(a.version) > 0)
  } catch {
...
const enabledMcpTools = computed(() => (mcpTools.value || []).filter((t) => t.enabled !== false))
...
const subWorkflowDisplayText = computed(() => resolveSubWorkflowName(props.node?.data, publishedWorkflowAgents.value))
...
const batchErrorStrategy = computed(
  () => props.node.data?.error_strategy || props.node.data?.errorStrategy || 'continueOnError'
)
...
function onLoopArrayVariableChange(v) {
  ...
  if (!props.node.data.input_params)
    props.node.data.input_params = [{ key: 'item', type: 'Object', value_from: 'refer', value: v }]
...
.kb-config-field :deep(.ant-input-number) {
  width: 100%;
}
.kb-config-readonly-hint {
  margin-top: 10px;
  font-size: 11px;
  color: var(--color-mute);
}
...
.builtin-vars-form-item {
  margin-bottom: 8px !important;
}
.builtin-vars-form-item :deep(.ant-form-item-label) {
  padding-bottom: 4px;
}
.builtin-vars-inline {
  display: flex;
...
.builtin-var-tag code {
  font-size: 11px;
  color: var(--color-link);
}
.builtin-var-tag .copy-icon {
  font-size: 10px;
  color: var(--color-mute);
}
