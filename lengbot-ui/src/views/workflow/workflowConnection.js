...
function isAllowedGroupBoundaryConnection(sourceNode, targetNode, sourceType, targetType) {
  ...
  if (GROUP_END_BUILTIN.has(sourceType) && GROUP_START_BUILTIN.has(targetType) && sp != null && tp != null && sp !== tp)
    return true
  return false
}
...
export function isValidWorkflowConnection(connection, ctx) {
  ...
  const dup = edges.some((e) => {
    ...
    return e.source === source && e.target === target && (e.sourceHandle || HANDLE_OUT) === (sourceHandle || HANDLE_OUT)
  })
  if (dup) return false
