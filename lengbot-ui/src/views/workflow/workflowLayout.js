...
function getBranchHandleRank(node, sourceHandle) {
  ...
  if (node.type === 'classifier') {
    ...
    const intentId = sourceHandle.startsWith(prefix) ? sourceHandle.slice(prefix.length) : sourceHandle
...
function minimizeCrossings(layerMap, edges, nodeMap) {
  ...
  for (const lv of sortedLayerKeys) {
    currentOrders.set(
      lv,
      deterministicSort(layerMap.get(lv)).map((n) => n.id)
    )
  }
  ...
  for (let i = 0; i < sortedLayerKeys.length - 1; i++) {
    ...
    bestCrossings += countCrossings(
      ...
      layerMap.get(lvB),
      edges,
      currentOrders.get(lvA),
      currentOrders.get(lvB)
    )
  }
  ...
  for (let iter = 0; iter < CROSSING_REDUCTION_ITERATIONS; iter++) {
    ...
    for (let i = 0; i < sortedLayerKeys.length - 1; i++) {
      const lvA = sortedLayerKeys[i]
      const lvB = sortedLayerKeys[i + 1]
      crossings += countCrossings(layerMap.get(lvA), layerMap.get(lvB), edges, newOrders.get(lvA), newOrders.get(lvB))
    }
...
function centerBranchNodesAmongTargets(positions, layerMap, edges, nodeMap) {
  ...
  for (const node of branchNodes) {
    const targets = edges.filter((e) => e.source === node.id && positions[e.target]).map((e) => e.target)
...
export function formatWorkflowLayout(nodes, edges) {
  ...
  const topPositions = layoutNodesInScope(topNodes, projectEdgesToTopLevel(edgeList, result, topIdSet), TOP_ORIGIN)
