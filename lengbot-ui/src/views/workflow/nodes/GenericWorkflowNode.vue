<template>
  <div class="workflow-node generic-node" :class="nodeClass" @dblclick="$emit('edit')">
    ...
    <div class="node-body">
      <div v-if="summaryText" class="node-config">
        <span class="config-value">{{ summaryText }}</span>
      </div>
...
.generic-node.debug-executing {
  animation: wf-executing 1.2s linear infinite;
  border-color: var(--color-link);
}
.generic-node.debug-success {
  border-color: #22c55e;
  box-shadow: 0 0 0 3px rgba(34, 197, 94, 0.2);
}
.generic-node.debug-fail {
  border-color: #ef4444;
