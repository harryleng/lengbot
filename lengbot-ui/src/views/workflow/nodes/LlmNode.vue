<template>
  <div class="workflow-node llm-node" :class="nodeClass" @dblclick="$emit('edit')">
    ...
    <div class="node-body">
      ...
      <div v-if="!data.modelId && !data.promptTemplate" class="node-placeholder">点击配置模型和提示词</div>
    </div>
  </div>
</template>
...
.llm-node.debug-executing {
  animation: wf-exec 1.2s linear infinite;
  border-color: var(--color-link);
}
.llm-node.debug-success {
  border-color: #22c55e;
