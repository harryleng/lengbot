...
export const FIELD_HINTS = {
  ...
  llm: {
    ...
    enableStreaming:
      '开启后 LLM 逐 token 流式生成；若下游有「流程输出」节点，则仅 output 节点写入对话正文，LLM 作为中间态不重复展示',
  },
  classifier: {
  ...
  tool: {
    ...
    outputMappings:
      '出参映射：key 为写入流程的变量名，value 引用工具返回 JSON 字段，如 {{answer}}、{{results}}、{{output}}',
  },
  script: {
  ...
  confirm: {
    ...
    formFields:
      '确认表单字段：info=展示信息，radio/select=逐项添加选项，text/textarea/number=输入；提交后 key 写入流程变量（_ 开头不入库）',
  },
  output: {
