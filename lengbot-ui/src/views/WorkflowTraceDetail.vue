    ...
    <div class="detail-page__body">
      ...
      <a-spin :spinning="loading">
        ...
        <div v-if="viewMode === 'text' && nodeSpans.length" class="text-container">
          <div
            v-for="(span, idx) in nodeSpans"
            :key="span.spanId"
            class="text-node-card"
            :class="{ 'node-success': span.status === 'completed', 'node-error': span.status === 'failed' }"
          >
            ...
            <div v-if="textExpanded.has(span.spanId)" class="tn-body">
              ...
              <div v-if="llmMessages(span)" class="tn-section">
                ...
                <div class="llm-messages-list">
                  <div
                    v-for="(msg, mi) in llmMessages(span)"
                    :key="mi"
                    class="llm-msg-item"
...
import {
  spansToNodeStates,
  spansToReplayEvents,
  orderNodeSpansForReplay,
  buildExecutedEdgeIds,
} from './workflow/workflowViewerAdapter.js'
...
.reply-content-box :deep(h3) {
  margin-top: 12px;
  margin-bottom: 8px;
  font-weight: 600;
}
.reply-content-box :deep(pre) {
...
.reply-content-box :deep(code) {
  background: var(--color-canvas-soft);
  padding: 1px 4px;
  border-radius: 3px;
  font-size: 12px;
}
.reply-content-box :deep(pre code) {
  background: none;
  padding: 0;
}
.reply-content-box :deep(table) {
  border-collapse: collapse;
  margin: 8px 0;
  width: 100%;
}
.reply-content-box :deep(th),
.reply-content-box :deep(td) {
  border: 1px solid var(--color-border-slate);
  padding: 6px 10px;
  font-size: 12px;
}
...
.reply-content-box :deep(ol) {
  padding-left: 20px;
  margin: 6px 0;
}
.reply-content-box :deep(blockquote) {
  border-left: 3px solid #d1d5db;
  padding-left: 12px;
  color: var(--color-mute);
  margin: 8px 0;
}
.reply-content-box :deep(a) {
  color: #1890ff;
  text-decoration: none;
}
.reply-content-box :deep(a:hover) {
...
.text-node-card.node-success {
  border-left: 4px solid #52c41a;
}
.text-node-card.node-error {
...
.llm-msg-system {
  ...
  border-color: var(--color-warn-bg-deep);
}
.llm-msg-user {
  background: #e6f7ff;
  border-color: #91d5ff;
