  ...
  <div class="session-manage">
    <div class="page-header">
      ...
      <div class="page-header-right">
        ...
        <a-input v-model:value="searchText" placeholder="搜索会话名称..." allow-clear style="width: 220px">
    ...
    <!-- 会话详情抽屉 -->
    <a-drawer v-model:open="detailVisible" :title="detailSession?.title || '会话详情'" :width="640" :footer="null">
        ...
        <a-spin :spinning="messagesLoading">
          ...
          <div v-else class="detail-messages-list">
            ...
            <div v-if="hasMoreMessages" class="detail-load-more">
              <a-button size="small" :loading="loadingOlder" @click="loadOlderMessages">加载更早的消息</a-button>
            </div>
          </div>
        </a-spin>
      </template>
    ...
    <!-- 消息元数据弹窗 -->
    <a-modal v-model:open="metaModalVisible" title="消息元数据" :footer="null" :width="600">
...
async function handleBatchDeleteMessages() {
  ...
  if (count === 0) return
  try {
    await Promise.all(selectedMsgKeys.value.map((id) => deleteMessage(detailSession.value.id, id).catch(() => null)))
