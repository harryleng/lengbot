      ...
      <div v-for="(inst, idx) in instances" :key="inst.id" class="instance-card">
        ...
        <div class="instance-header">
          ...
          <div class="instance-actions">
            <a-tooltip title="清空对话" :get-popup-container="(t) => t.parentElement" v-if="inst.messages.length > 0">
              <button class="btn-icon-sm" @click="clearChat(inst)">
                <ClearOutlined />
              </button>
            </a-tooltip>
            <a-tooltip title="复制配置" :get-popup-container="(t) => t.parentElement">
              <button class="btn-icon-sm" @click="addInstance(inst)" :disabled="instances.length >= 3">
                <CopyOutlined />
              </button>
            </a-tooltip>
            <a-tooltip title="删除配置" :get-popup-container="(t) => t.parentElement" v-if="instances.length > 1">
    ...
    if (detail) {
      ...
      if (detail.variables) {
        ...
        if (Array.isArray(vars)) {
          ...
          inst.variables = Object.entries(vars).map(([key, defaultValue]) => ({
            key,
