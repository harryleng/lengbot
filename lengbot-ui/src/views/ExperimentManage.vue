<template>
  <div class="page">
        ...
        <template v-if="column.key === 'action'">
          <div class="table-actions">
            <a-tooltip v-if="(record.status?.code || record.status) === 'running'" title="停止">
              <button class="btn-icon" @click="handleStop(record.id)">
                <PauseCircleOutlined />
              </button>
            </a-tooltip>
            <a-tooltip v-if="(record.status?.code || record.status) === 'stopped'" title="重启">
              <button class="btn-icon" @click="handleRestart(record.id)">
                <PlayCircleOutlined />
              </button>
            </a-tooltip>
            <a-tooltip title="删除">
              <button class="btn-icon danger" @click="handleDelete(record.id)">
                <DeleteOutlined />
              </button>
            </a-tooltip>
          </div>
    ...
    <!-- 创建实验弹窗 -->
    <a-modal v-model:open="createDialogVisible" title="创建实验" :width="720" :footer="null" :mask-closable="false">
      <ExperimentCreateForm ref="createFormRef" @success="onCreateSuccess">
        <template #cancel>
...
import { getExperiments, stopExperiment, deleteExperiment, restartExperiment } from '../api/experiment'
