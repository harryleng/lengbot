<template>
  <div class="detail-page">
    ...
    <!-- 新建版本弹窗 -->
    <a-modal v-model:open="versionDialogVisible" title="新建版本" :width="480" :mask-closable="false">
      <p style="color: var(--color-mute); margin-bottom: 16px">将当前数据集中的所有数据项快照为一个新版本。</p>
      ...
      <template #footer>
        <LbDialogFooter :loading="submitting" @cancel="versionDialogVisible = false" @confirm="handleCreateVersion" />
      </template>
    </a-modal>

    <!-- 添加数据项弹窗 -->
    <a-modal v-model:open="itemDialogVisible" title="添加数据项" :width="640" :mask-closable="false">
      <div class="dialog-scroll-body">
        <a-form :model="itemForm" :label-col="{ flex: '0 0 100px' }">
          <a-form-item label="输入内容" required>
            <a-textarea
              v-model:value="itemForm.input"
              :rows="4"
              :maxlength="2000"
              show-count
              placeholder="用户输入内容 (不超过2000字)"
            />
          </a-form-item>
          <a-form-item label="期望输出">
            <a-textarea
              v-model:value="itemForm.referenceOutput"
              :rows="4"
              :maxlength="2000"
              show-count
              placeholder="期望的输出结果 (不超过2000字)"
            />
          </a-form-item>
      ...
      <template #footer>
        <LbDialogFooter :loading="submitting" @cancel="itemDialogVisible = false" @confirm="handleCreateItem" />
      </template>
    </a-modal>
...
import {
  getEvalDataset,
  getEvalDatasetVersions,
  createEvalDatasetVersion,
  getEvalDatasetVersionItems,
  getEvalDatasetItems,
  createEvalDatasetItem,
