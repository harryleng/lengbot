<template>
  <div class="page">
    ...
    <!-- 新增/编辑弹窗 -->
    <a-modal v-model:open="dialogVisible" :width="720" :mask-closable="false">
      <template #title>
        ...
        <a-form :model="form" :label-col="{ flex: '0 0 120px' }">
          <a-form-item label="slug" required v-if="!form.id || form.scope === 'global'">
            <a-input
              ...
              placeholder="英文-小写-短横线，如 deep-research（不超过30字）"
              :maxlength="30"
              show-count
              :disabled="form.id && form.isBuiltin === 1"
            />
          </a-form-item>
          <a-form-item label="技能名称" required>
            <a-input
              v-model:value="form.name"
              placeholder="英文短名，对模型可读，如 deep_research（不超过30字）"
              :maxlength="30"
              show-count
            />
          </a-form-item>
          ...
          <a-form-item label="依赖工具">
            <a-select
              ...
              mode="multiple"
              placeholder="选择该 Skill 启用时附带的工具"
              style="width: 100%"
          ...
          <a-form-item label="依赖 MCP Server">
            <a-select
              v-model:value="form.mcpServerIds"
              mode="multiple"
              placeholder="选择该 Skill 启用时附带的 MCP Server"
              style="width: 100%"
              option-label-prop="label"
            >
    ...
    <!-- ZIP 导入弹窗 -->
    <SkillImportModal v-model:open="importModalVisible" @imported="loadData" />

    <!-- 远程安装弹窗 -->
    <SkillRemoteInstallModal v-model:open="remoteInstallVisible" @installed="loadData" />
    ...
    <a-modal v-model:open="guideVisible" title="Skill 说明" :width="640" :footer="null">
      <div class="guide">
        <div class="guide-section">
          ...
          <p>
            Skill 是
            <strong>可复用的能力包</strong>
            ：把「何时启用、如何执行」写成提示词模板，并可附带依赖的工具与 MCP Server。在 Agent
            详情中启用后，对话时会把这些 Skill 注入系统上下文，引导主模型按场景选用对应能力（与 Yuxi 的 Skill
            中间件思路一致）。
          </p>
          <p>
            与 SubAgent 的区别：Skill 是
            <strong>提示词 + 工具扩展</strong>
            ，由主 Agent 在同一轮对话中执行；SubAgent 是
            <strong>独立子智能体</strong>
            ，通过委派工具异步完成子任务。
          </p>
        </div>
        <div class="guide-section">
          ...
          <div class="guide-step">
            ...
            <div>
              <b>填写 slug 与名称</b>
              <p>slug 为全局唯一英文短横线标识；name 为模型可读英文名；displayName 为界面展示名。</p>
            </div>
          </div>
          <div class="guide-step">
            ...
            <div>
              <b>编写提示词模板</b>
              <p>建议包含「触发条件」与「执行规则」，例如深度研究、知识库问答等场景说明。</p>
            </div>
          </div>
          <div class="guide-step">
            ...
            <div>
              <b>（可选）绑定依赖</b>
              <p>选择该 Skill 启用时需要一并开放的工具或 MCP Server。</p>
            </div>
          </div>
          <div class="guide-step">
            ...
            <div>
              <b>在 Agent 中启用</b>
              <p>进入智能体详情 → Skill Tab，从列表勾选启用（最多 10 个）。发布版本后绑定关系会写入版本快照。</p>
            </div>
          </div>
        </div>
        <div class="guide-section">
...
async function openDialog(row) {
  ...
  if (row) {
    ...
    selectedToolIds
      .filter((id) => !existingToolIds.has(id))
      .forEach((id) => {
