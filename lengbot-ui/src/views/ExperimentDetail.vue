<template>
  <div class="detail-page">
    ...
    >
      ...
      <template #extra>
        <button v-if="experimentStatus === 'running'" class="lb-btn" @click="handleStop">
          <PauseCircleOutlined />
          停止
        </button>
        <button
      ...
      <div class="info-cards">
        <div class="info-card">
          ...
          <div class="info-value">
            ...
            <a v-if="experiment?.datasetId" @click="router.push(`/app/eval/datasets/${experiment.datasetId}`)">
        ...
        <div class="info-card">
          ...
          <div class="info-value">
            ...
            <a v-if="experiment?.promptKey" @click="router.push(`/app/prompts/${experiment.promptKey}`)">
              {{ experiment.promptKey }}
            </a>
            <span v-else>-</span>
          </div>
        </div>
        <div class="info-card">
          ...
          <div class="info-value" v-if="experiment?.evaluatorNameList?.length">
            <div
              v-for="(name, i) in experiment.evaluatorNameList"
              :key="(experiment.evaluatorIdList?.[i] || name) + '-' + i"
              class="evaluator-info-item"
            >
              <a-tag v-if="experiment.evaluatorVersionList?.[i]" color="blue" size="small">
                {{ experiment.evaluatorVersionList[i] }}
              </a-tag>
          ...
          <a-tab-pane key="overview" tab="概览">
            <a-spin :spinning="overviewLoading" style="min-height: 200px; display: block">
              <div class="evaluator-cards" style="min-height: 200px">
                <div v-for="ev in evaluatorResults" :key="ev.evaluatorName" class="evaluator-card">
              ...
              <div v-if="evaluatorResults.length === 0 && !overviewLoading" class="empty-state">暂无评测结果</div>
            </a-spin>
          </a-tab-pane>

          <!-- 评测结果 Tab -->
          <a-tab-pane key="results" tab="评测结果">
            <a-tabs v-if="evaluatorResults.length" v-model:active-key="resultEvaluatorTab" type="card" size="small">
              <a-tab-pane v-for="ev in evaluatorResults" :key="ev.evaluatorName" :tab="ev.evaluatorName">
                <a-table
                  :data-source="getResultsForEvaluator(ev.evaluatorName)"
          ...
          <a-spin :spinning="restartFormLoading" tip="加载配置中...">
            <div class="restart-form-scroll">
              <a-form
                ...
                :style="{ opacity: restartFormLoading ? 0.4 : 1, transition: 'opacity 0.2s' }"
              >
                ...
                <a-form-item label="变量映射">
                  <a-textarea
                    v-model:value="editForm.variableMapping"
                    :rows="2"
                    placeholder='JSON: {"input":"user_input"}'
                  />
                </a-form-item>
                <div
                  v-for="(ev, idx) in editForm.evaluators"
                ...
                >
                  ...
                  <a-form-item label="评估器" required>
                    <a-select
                      ...
                      placeholder="选择评估器"
                      @change="(id) => onEditEvaluatorChange(idx, id)"
                    >
                  ...
                  <a-form-item label="参数映射">
                    <a-textarea
                      ...
                      :rows="2"
                      placeholder='JSON: {"actual_output":"output"}'
                    />
                  </a-form-item>
                </div>
                <a-button
                  v-if="editForm.evaluators.length < 5"
                  type="dashed"
                  size="small"
                  block
                  @click="addEditEvaluator"
    ...
    <!-- 结果详情弹窗 -->
    <a-modal v-model:open="detailModalVisible" title="评测结果详情" :width="720" :footer="null" :mask-closable="false">
...
function initEditForm() {
  ...
  try {
    ...
    if (varMap.length > 0) {
      const mapped = {}
      varMap.forEach((m) => {
        mapped[m.promptVariable] = m.datasetColumn
      })
  ...
  try {
    ...
    editForm.evaluators = evalConfigs.map((cfg) => {
      ...
      if (eVarMap.length > 0) {
        const mapped = {}
        eVarMap.forEach((m) => {
          mapped[m.evaluatorVariable] = m.source
        })
...
async function onEditEvaluatorChange(idx, id) {
  ...
  try {
    ...
    editForm.evaluators[idx].versions = []
  }
}
...
.evaluator-score.score-high .score-value {
  color: #52c41a;
}
.evaluator-score.score-mid .score-value {
  color: #faad14;
}
.evaluator-score.score-low .score-value {
  color: #ff4d4f;
}
.evaluator-meta {
  margin-top: 8px;
