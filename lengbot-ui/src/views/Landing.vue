<template>
  <div class="landing-container">
    ...
    <div v-if="isBackendDown" class="loading-container">
      <img src="/lengbot-logo-single.png" alt="LengBot" class="error-logo" />
      <h2 class="error-title">服务连接失败</h2>
      <p class="error-desc">无法连接到 LengBot 后端服务，请确认服务已启动。</p>
    ...
    <template v-else>
      ...
      <header class="glass-header">
        <div class="logo" @click="router.push('/')">
          <img src="/lengbot-logo-single.png" alt="LengBot" class="logo-img" />
          <span class="logo-text">LengBot</span>
        </div>
      ...
      <main class="hero-section">
        <div class="hero-layout">
          ...
          <div class="hero-content">
            <h1 class="title reveal-up delay-1">{{ config.title || 'LengBot' }}</h1>
      ...
      <!-- 底部 -->
      <footer class="footer">
        <p class="copyright">{{ config.copyright || '© 2026 LengBot. All Rights Reserved.' }}</p>
      </footer>
    </template>
  </div>
</template>
...
.orb-2 {
  ...
  bottom: -180px;
  left: -120px;
  background: radial-gradient(circle, rgba(121, 40, 202, 0.1), rgba(168, 85, 247, 0.03) 70%);
...
.orb-3 {
  ...
  top: 35%;
  left: 50%;
  background: radial-gradient(circle, rgba(56, 189, 248, 0.1), rgba(0, 112, 243, 0.03) 70%);
...
.button-base.primary {
  ...
  color: #fff;
  box-shadow:
    0 8px 24px -6px rgba(0, 112, 243, 0.45),
...
.button-base.primary:hover {
  ...
  box-shadow:
    0 14px 32px -6px rgba(0, 112, 243, 0.55),
...
.visual-card {
  ...
  border: 1px solid var(--color-hairline);
  box-shadow:
    0 24px 48px -20px rgba(0, 112, 243, 0.12),
    var(--shadow-4);
  overflow: hidden;
}
.visual-glow {
  ...
  width: 70%;
  height: 70%;
  background: radial-gradient(circle, rgba(0, 112, 243, 0.1), rgba(56, 152, 236, 0.03) 60%, transparent 80%);
  pointer-events: none;
}
