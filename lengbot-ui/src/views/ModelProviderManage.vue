  ...
  <div class="page">
    ...
    >
      <template #actions>
        <button class="lb-btn" :disabled="refreshing" @click="handleRefreshCache">
          <SyncOutlined :class="{ spinning: refreshing }" />
          刷新缓存
        </button>
      </template>
    </LbManageHeader>
    ...
    <div class="provider-grid">
      <div
        v-for="p in list"
        :key="p.id"
        :class="['provider-card', { disabled: p.status?.code === 'disabled' || p.status === 'disabled' }]"
      >
        <div class="card-top">
        ...
        <div class="card-footer">
          <button
            class="btn-link"
            :disabled="p.status?.code === 'disabled' || p.status === 'disabled'"
            @click="openModelModal(p)"
          >
            管理模型
          </button>
        </div>
    ...
    <a-modal v-model:open="presetModalVisible" title="选择提供商预设" :width="760" :mask-closable="false">
    ...
    <a-modal
      ...
      :title="`${currentProvider?.name || ''} - 联网拉取`"
      :width="760"
      :footer="null"
      :mask-closable="false"
    >
      <div class="fetch-tabs">
        <button :class="['fetch-tab', { active: fetchTab === 'available' }]" @click="fetchTab = 'available'">
          可添加
          <span class="fetch-tab-count">{{ fetchedModels.length }}</span>
        </button>
        <button :class="['fetch-tab', { active: fetchTab === 'existing' }]" @click="fetchTab = 'existing'">
          已有模型
          <span class="fetch-tab-count">{{ modelList.length }}</span>
        </button>
      </div>
      ...
      <div class="fetch-modal-footer">
        ...
        <div class="fetch-footer-right">
          ...
          <button
            v-if="fetchTab === 'available'"
            class="btn-primary-sm"
            :disabled="selectedFetchedModels.length === 0 || modelSubmitting"
            @click="handleBatchAddModels"
          >
...
import {
  ...
  updateModelProvider,
  deleteModelProvider,
  checkModelProviderByForm,
  fetchProviderModels,
  refreshModelProviderCache,
...
const filteredFetchedModels = computed(() => {
  ...
  if (fetchSearchText.value) {
    ...
    result = result.filter(
      (m) => m.modelId.toLowerCase().includes(keyword) || (m.name || '').toLowerCase().includes(keyword)
    )
  }
  return result
})
...
const fetchTypeTabsCache = computed(() => {
  ...
  return [{ value: 'all', label: '全部', count: counts.all }, ...tabs].filter((t) => t.count > 0 || t.value === 'all')
})
...
const currentFetchTabs = computed(() =>
  fetchTab.value === 'available' ? fetchTypeTabsCache.value : existingTypeTabs.value
)
...
const defaultModelPlaceholder = computed(() =>
  providerDefaultModelId.value ? `不填则默认为${providerDefaultModelId.value}模型` : '不填则使用默认模型'
)
