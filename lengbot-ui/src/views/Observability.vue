  ...
  <div class="observability">
    ...
    <div v-if="activeTab !== 'tool'" class="filter-bar">
      ...
      <a-range-picker
        ...
        :style="{ width: '360px' }"
      />
      <a-button type="primary" @click="loadTraces(1)">
        <SearchOutlined />
        查询
      </a-button>
    ...
    <div v-if="activeTab === 'tool'" class="filter-bar">
      <a-input
        v-model:value="toolFilter.toolName"
        placeholder="工具名称"
        style="width: 180px"
        allow-clear
        @press-enter="loadToolCalls(1)"
      />
      <a-select v-model:value="toolFilter.status" placeholder="状态" style="width: 120px" allow-clear>
      ...
      <a-range-picker
        ...
        style="width: 380px"
      />
      <a-button type="primary" @click="loadToolCalls(1)">
        <SearchOutlined />
        查询
      </a-button>
      <a-button @click="handleToolRefresh">
        <ReloadOutlined />
        刷新
      </a-button>
      <a-button danger :disabled="selectedToolRowKeys.length === 0" @click="handleBatchDeleteTool">
        <DeleteOutlined />
        批量删除
      </a-button>
    </div>
    ...
    <a-table
      v-if="activeTab === 'tool'"
      :data-source="toolRecords"
      :columns="toolColumns"
      :loading="toolLoading"
      :pagination="toolPagination"
      :row-selection="{ selectedRowKeys: selectedToolRowKeys, onChange: (keys) => (selectedToolRowKeys = keys) }"
      row-key="id"
      size="small"
    ...
    <!-- 工具调用 详情弹窗 -->
    <a-modal v-model:open="toolDetailVisible" title="工具调用详情" :width="720" :footer="null" :mask-closable="false">
      <div v-if="toolDetailRecord" class="dialog-scroll-body">
        <a-descriptions :column="2" size="small" bordered style="margin-bottom: 16px">
          <a-descriptions-item label="工具名称">
            <ToolOutlined />
            {{ toolDetailRecord.toolName }}
          </a-descriptions-item>
          <a-descriptions-item label="状态">
            <a-tag
              :color="
                toolDetailRecord.status === 'success'
                  ? 'success'
                  : toolDetailRecord.status === 'error'
                    ? 'error'
                    : 'processing'
              "
            >
          ...
          <a-descriptions-item label="调用时间" :span="2">
            {{ formatTime(toolDetailRecord.createdAt) }}
          </a-descriptions-item>
    ...
    <!-- Trace 详情抽屉 -->
    <a-drawer v-model:open="detailVisible" :width="860" placement="right">
      <template #title>
        <div
          style="display: flex; align-items: center; justify-content: space-between; width: 100%; padding-right: 40px"
        >
          <span>Trace 详情</span>
      ...
      <template v-if="detailTrace">
        ...
        <div v-if="traceModelInput.hasData" class="model-input-section">
          ...
          <div
            v-if="traceModelInput.userContent || traceModelInput.userAttachments.length || traceModelInput.bizParams"
            class="mi-block"
          >
            <div class="mi-block-title-row">
              <span class="mi-block-title">本轮用户输入</span>
              <button
                v-if="traceModelInput.userContent"
                class="btn-copy-sm"
                @click="copyToClipboard(traceModelInput.userContent, 'mi_user')"
              >
            ...
            <div v-if="traceModelInput.userAttachments.length" class="trace-att-thumbs">
              <MediaAttachmentThumb v-for="(att, ai) in traceModelInput.userAttachments" :key="'ua-' + ai" :att="att" />
            </div>
          </div>
          ...
          <div v-if="traceModelInput.llmMessages.length" class="mi-block">
            ...
            <div
              v-for="(m, mi) in traceModelInput.llmMessages"
              :key="'lm-' + mi"
              class="mi-msg"
              :class="{
            ...
            >
              ...
              <div v-if="m.content" class="mi-pre">
                <MentionTextRenderer
                  v-if="
                    m.role === 'user' &&
                    shouldShowTraceMentions(m.content, resolveTraceMessageMentions(m, traceModelInput.userMentions))
                  "
                  :content="m.content"
          ...
          <div class="waterfall-container" v-if="waterfallGroups.length > 0">
            <div class="waterfall-header">
              ...
              <span class="wf-bar-area">
                <span v-for="p in [0, 25, 50, 75, 100]" :key="p" class="wf-tick" :style="{ left: p + '%' }">
                  {{ p }}%
                </span>
              </span>
              ...
              <div v-if="expandedSpans.has(group.spanId)" class="span-inline-detail">
                ...
                <div v-if="group.spans.length > 1" class="sd-section">
                  <div class="sd-section-title">
                    汇总（共 {{ group.spans.length }} 次调用，总耗时 {{ formatDuration(group.totalDurationMs) }}）
                  </div>
                </div>
                <!-- 每个子段的详情 -->
                <div v-for="(sub, si) in group.spans" :key="sub.spanId + '_' + si" class="span-sub-detail">
                  <div v-if="group.spans.length > 1" class="sd-sub-header">
                    ...
                    <span class="sd-sub-time">
                      {{ formatDuration(sub.durationMs) }}（占比
                      {{ ((sub.durationMs / group.totalDurationMs) * 100).toFixed(0) }}%）
                    </span>
                  </div>
                  <div class="sd-grid">
                    <div class="sd-item">
                      <span class="sd-key">名称</span>
                      <span class="sd-val">{{ sub.name }}</span>
                    </div>
                    <div class="sd-item">
                      <span class="sd-key">Span ID</span>
                      <span class="sd-val">{{ sub.spanId }}</span>
                    </div>
                    <div class="sd-item">
                      <span class="sd-key">父Span</span>
                      <span class="sd-val">{{ sub.parentSpanId || '-' }}</span>
                    </div>
                    <div class="sd-item">
                      <span class="sd-key">状态</span>
                      <span class="sd-val">{{ sub.status }}</span>
                    </div>
                  </div>
                  <!-- AI回复内容 -->
                  ...
                  <div v-if="sub.attributes?.content && sub.name === 'ai_reasoning'" class="sd-section">
                    <div class="sd-section-title-row">
                      ...
                      <button
                        class="btn-copy-sm"
                        @click="copyToClipboard(sub.attributes.content, 'reasoning_' + sub.spanId + '_' + si)"
                      >
                        <CheckOutlined
                          v-if="copiedKey === 'reasoning_' + sub.spanId + '_' + si"
                  ...
                  <div v-if="sub.attributes?.content && sub.name === 'ai_reply'" class="sd-section">
                    <div class="sd-section-title-row">
                      ...
                      <button
                        class="btn-copy-sm"
                        @click="copyToClipboard(sub.attributes.content, 'reply_' + sub.spanId + '_' + si)"
                      >
                        <CheckOutlined
                          v-if="copiedKey === 'reply_' + sub.spanId + '_' + si"
                          style="color: var(--green-500)"
                        />
                        <CopyOutlined v-else />
                      </button>
                  ...
                  <div v-if="sub.name === 'user_message'" class="sd-section">
                    ...
                    <div v-if="sub.attributes?.content" class="sd-content-box">
                      <MentionTextRenderer
                        v-if="
                          shouldShowTraceMentions(
                            sub.attributes.content,
                            normalizeTraceMentions(sub.attributes.mentions)
                          )
                        "
                    ...
                    <div
                      v-if="traceAttachments(sub.attributes).length"
                  ...
                  <div
                    v-if="sub.name === 'messages_to_llm' && traceLlmMessages(sub.attributes).length"
                    class="sd-section"
                  >
                    <div class="sd-section-title">
                      发送给模型的消息（{{ traceLlmMessages(sub.attributes).length }} 条，详见上方完整输入区）
                    </div>
                  </div>
                  <!-- 其他属性 -->
                  <div
                    v-if="sub.attributes && Object.keys(sub.attributes).filter((k) => !traceHiddenAttrKeys(k)).length"
                    class="sd-section"
                  >
    ...
    <!-- 请求配置弹窗 -->
    <a-modal v-model:open="configModalVisible" title="请求配置" :width="600" :footer="null" :mask-closable="false">
    ...
    <a-modal
      ...
      :footer="null"
      :mask-closable="false"
      @after-close="onToolsModalClosed"
    >
      <a-input v-model:value="toolSearchKeyword" placeholder="搜索工具标识" allow-clear class="tool-def-search">
      ...
      <div class="dialog-scroll-body">
        ...
        >
          ...
          <pre v-if="tool.inputSchema" class="mi-pre" style="margin-top: 6px; max-height: 150px">{{
...
function normalizeTraceMentions(raw, content = '') {
  if (!Array.isArray(raw) || raw.length === 0) {
    return contentHasMentionTokens(content)
      ? content
          .match(/@(knowledge|subagent|skill|tool):(\d+)/g)
          ?.map((token) => {
...
const waterfallGroups = computed(() => {
  ...
  function calcDepth(s) {
    let depth = 0,
      pid = s.parentSpanId
    while (pid && spanMap.has(pid)) {
      depth++
      pid = spanMap.get(pid).parentSpanId
    }
    return depth
  }
  ...
  return sortedEntries.map(([spanId, group]) => {
    ...
    const subSegments = group.map((s) => {
      ...
      const subWidthPercent =
        groupDuration > 0 ? Math.min(100 - subOffsetPercent, ((s.durationMs || 0) / groupDuration) * 100) : 100
...
function formatAttrs(attrs) {
  const filtered = Object.fromEntries(Object.entries(attrs).filter(([k]) => !traceHiddenAttrKeys(k)))
...
.tool-icon {
  background: linear-gradient(135deg, var(--color-warning), var(--color-warning-deep));
}
.stat-value {
  font-size: 22px;
  font-weight: 600;
...
.wf-bar-tool {
  background: linear-gradient(90deg, var(--color-warning), var(--color-warning-deep));
}
.wf-bar-rag {
...
.wf-bar-askuser {
  background: linear-gradient(90deg, var(--color-warning), #ffa940);
}
.wf-bar-other {
