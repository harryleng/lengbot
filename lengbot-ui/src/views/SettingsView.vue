<template>
  <div class="page">
          ...
          <div class="panel-body">
            <a-form :label-col="{ span: 4 }" :wrapper-col="{ span: 18 }">
              <a-form-item label="主标题">
                <a-input v-model:value="landing.title" placeholder="LengBot" :maxlength="30" show-count />
              </a-form-item>
              <a-form-item label="副标题轮播">
                <div class="subtitle-list">
                  <div
                    v-for="(sub, idx) in landing.subtitles"
                    :key="(sub || 'subtitle') + '-' + idx"
                    class="subtitle-row"
                  >
                    <a-input
                      v-model:value="landing.subtitles[idx]"
                      placeholder="副标题"
                      style="flex: 1"
                      :maxlength="30"
                      show-count
              ...
              <a-form-item label="描述文字">
                <a-textarea
                  v-model:value="landing.description"
                  :rows="3"
                  placeholder="平台介绍文字"
                  :maxlength="200"
                  show-count
              ...
              <a-form-item label="版权信息">
                <a-input v-model:value="landing.copyright" placeholder="© 2026 LengBot" :maxlength="100" show-count />
              </a-form-item>
              <a-form-item label="功能展示">
                ...
                <div v-show="featuresExpanded" class="feature-list">
                  <div
                    v-for="(feat, idx) in landing.features"
                    :key="(feat.title || feat.icon || 'feature') + '-' + idx"
                    class="feature-card"
                  >
                    <div class="feature-card-header">
                      ...
                      <div class="feature-card-actions">
                        ...
                        <a-tooltip title="下移">
                          <button
                            class="btn-icon-move"
                            :disabled="idx === landing.features.length - 1"
                    ...
                    <a-form-item label="描述" :label-col="{ span: 3 }" :wrapper-col="{ span: 20 }">
                      <a-textarea
                        v-model:value="feat.desc"
                        :rows="2"
                        placeholder="功能描述（建议不超过40字）"
                        :maxlength="40"
                        show-count
                      />
                    </a-form-item>
              ...
              <a-form-item :wrapper-col="{ offset: 4, span: 18 }">
                <button class="btn-primary" :disabled="landingSaving" @click="saveLandingConfig">
                  <SaveOutlined />
                  {{ landingSaving ? '保存中...' : '保存 Landing 配置' }}
                </button>
              </a-form-item>
            </a-form>
          </div>
            ...
            <div class="panel-body">
              <div class="token-stat-cards">
                <div class="token-stat-card">
                  ...
                  <a-progress
                    :percent="
                      tokenStats.globalLimit ? Math.min(100, (tokenStats.globalUsed / tokenStats.globalLimit) * 100) : 0
                    "
            ...
            <div class="panel-body">
              <a-form :label-col="{ span: 8 }">
                <a-form-item label="单次调用上限">
                  <a-input-number
                    v-model:value="tokenConfig.singleCallLimit"
                    :min="1000"
                    :step="1000"
                    style="width: 100%"
                    addon-after="tokens"
                ...
                <a-form-item label="用户日限额">
                  <a-input-number
                    v-model:value="tokenConfig.userDailyLimit"
                    :min="10000"
                    :step="100000"
                    style="width: 100%"
                    addon-after="tokens"
                ...
                <a-form-item label="全局日限额">
                  <a-input-number
                    v-model:value="tokenConfig.globalDailyLimit"
                    :min="100000"
                    :step="1000000"
                    style="width: 100%"
                    addon-after="tokens"
    ...
    <div v-show="activeTab === 'apiKey'">
      <a-spin :spinning="apiKeyLoading">
        <div class="panel" style="grid-column: 1 / -1">
              ...
              <a-popover trigger="click" placement="right" :overlay-style="{ maxWidth: '480px' }">
                <template #content>
                  <div style="font-size: 13px; line-height: 1.8; padding: 4px 0">
                    ...
                    <p><b>什么是 API Key？</b></p>
                    <p>
                      API Key 是用于外部系统调用 LengBot 接口的认证凭证，格式为
                      <code>lbkey_xxxx...</code>
                      ，支持通过 HTTP Header 传递。
                    </p>
                ...
                <QuestionCircleOutlined
                  style="font-size: 15px; color: var(--color-mute); cursor: pointer; margin-left: 6px"
                />
              </a-popover>
            </div>
            <span class="panel-desc">用于外部系统调用 LengBot 接口的认证凭证</span>
          ...
          <div class="panel-body">
            <div v-if="apiKeyList.length === 0 && !apiKeyLoading" class="empty-tip">
              暂无 API Key，点击上方按钮创建一个
            </div>
              ...
              <div v-for="key in apiKeyList" :key="key.id" class="apikey-card">
                ...
                <div class="apikey-card-body">
                  ...
                  <div class="apikey-card-row">
                    ...
                    <span>
                      {{ formatToken(key.dailyQuota) }} / 日{{
                        key.usedTokens > 0 ? '（已用 ' + formatToken(key.usedTokens) + '）' : ''
                      }}
                    </span>
                  </div>
                ...
                <div class="apikey-card-footer">
                  ...
                  <div class="apikey-card-actions">
                    <a-popconfirm
                      ...
                      @confirm="handleDeleteApiKey(key)"
                      ok-text="确定"
                      cancel-text="取消"
                    >
    ...
    <a-modal
      ...
      :confirm-loading="apiKeyCreateLoading"
      ok-text="创建"
      cancel-text="取消"
    >
      <div class="dialog-scroll-body">
        <a-form :label-col="{ flex: '0 0 120px' }">
          ...
          <a-form-item label="每分钟限流">
            <a-input-number
              v-model:value="apiKeyForm.rateLimit"
              :min="1"
              :max="10000"
              style="width: 100%"
              placeholder="60"
          ...
          <a-form-item label="每日Token配额">
            <a-input-number
              v-model:value="apiKeyForm.dailyQuota"
              :min="0"
              :max="100000000"
              :step="10000"
              style="width: 100%"
          ...
          <a-form-item label="绑定Agent">
            <AgentSelect v-model:value="apiKeyForm.agentIds" mode="multiple" placeholder="不选择则可访问全部Agent" />
          </a-form-item>
        </a-form>
      </div>
    </a-modal>
...
import {
  ...
  getTokenBudgetStats,
  getTokenBudgetRanking,
} from '../api/tokenBudget'
...
const modelSaveConfig = {
  ...
  embedding: {
    ...
    api: updateDefaultEmbeddingModel,
    label: '默认向量模型',
  },
  rerank: {
