      ...
      <div class="trend-row">
        <div class="panel panel-trend">
          ...
          <div class="chat-summary">
            <span>
              区间会话:
              <b>{{ chatStats.totalSessions ?? '-' }}</b>
            </span>
            <span>
              区间消息:
              <b>{{ chatStats.totalMessages ?? '-' }}</b>
            </span>
...
const trendBarLayout = computed(() => {
  ...
  if (!containerW || minScrollWidth <= containerW) {
    const colWidth =
      containerW > 0 ? Math.floor((containerW - Math.max(0, n - 1) * gap) / n) : Math.min(80, Math.round(480 / n))
...
const trendTrackStyle = computed(() => {
  const { fill, colWidth } = trendBarLayout.value
  const trackWidth = fill ? Math.max(28, Math.min(72, colWidth - 16)) : Math.max(28, Math.min(48, colWidth - 12))
...
[data-theme='dark'] .knowledge-icon {
  background: #0c1a3d;
}
...
.status-fill.published {
  background: #16a34a;
}
.status-fill.published_editing {
  background: #3b82f6;
}
...
.recent-tag.published {
  background: var(--color-success-bg);
  color: #16a34a;
}
.recent-tag.published_editing {
  background: var(--color-info-bg);
  color: #2563eb;
}
.recent-tag.archived {
...
[data-theme='dark'] .recent-tag.published {
  background: #052e16;
}
[data-theme='dark'] .recent-tag.published_editing {
  background: #0c1a3d;
}
[data-theme='dark'] .recent-tag.archived {
  background: #422006;
}
