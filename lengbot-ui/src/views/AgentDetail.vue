  ...
  <div class="detail-page" :class="{ 'detail-page--loading': pageLoading || saving || publishing }">
    ...
    >
      ...
      <template #extra>
        ...
        <button v-if="agent.agentType !== 'workflow'" class="lb-btn" @click="openVersionDrawer">
          <HistoryOutlined />
          版本管理
        </button>
        <button
              ...
              <div class="panel-body" :class="{ 'preview-lock-zone': isVersionPreview }">
                <a-form :model="agent" :label-col="{ span: 6 }">
                  ...
                  <a-form-item label="类型">
                    <a-select
                      v-model:value="agent.agentType"
                      style="width: 100%"
                      :disabled="!!agentId || isVersionPreview"
                    >
                  ...
                  <a-form-item label="欢迎语">
                    <a-textarea
                      v-model:value="agent.welcomeMessage"
                      :rows="2"
                      :maxlength="200"
                      show-count
                      placeholder="对话时显示的欢迎语（可选）"
                      :disabled="isVersionPreview"
                      :readonly="isVersionPreview"
                    />
                  </a-form-item>
                  <a-form-item label="推荐问题">
                    <div class="inline-field-block">
                      <div class="inline-field-toolbar">
                        <button
                          class="btn-ai-sm"
                          :disabled="generatingQuestions || isVersionPreview"
                      ...
                      <div v-if="recommendedQuestions.length === 0" class="sub-config-empty">
                        暂无推荐问题，最多 3 条
                      </div>
                      <div v-else class="config-list-scroll config-list-scroll--compact">
                        <div
                          ...
                          :key="i"
                          class="list-table-row list-table-row--2col"
                        >
                          <a-input
                            v-model:value="recommendedQuestions[i]"
                            placeholder="输入推荐问题（不超过 30 字）"
                            :maxlength="30"
                            size="small"
                            :disabled="isVersionPreview"
                          />
                          <a-tooltip title="删除">
                            <button
                              ...
                              :disabled="isVersionPreview"
                              @click="removeRecommendedQuestion(i)"
                            >
                              <CloseOutlined />
                            </button>
                          </a-tooltip>
            ...
            <div class="panel panel-stretch panel--config-unified">
              ...
              >
                      ...
                      <a-form-item v-if="agent.agentType !== 'workflow'" label="系统提示词">
                        <div class="prompt-wrapper">
                          ...
                          <a-tooltip :title="generatingPrompt ? '生成中...' : 'AI生成提示词'">
                            <button
                              class="btn-ai-icon"
                              :disabled="generatingPrompt || isVersionPreview"
                      ...
                      <div v-if="agent.agentType !== 'workflow'" class="sub-config-card">
                        ...
                        <template v-else>
                          ...
                          <div class="config-list-scroll">
                            <div
                              v-for="(v, idx) in promptVariables"
                              :key="v._id"
                              class="list-table-row list-table-row--4col"
                            >
                              <a-input
                                v-model:value="v.key"
                              ...
                              <a-tooltip title="删除">
                                <button
                                  ...
                                  class="btn-icon-sm danger"
                                  :disabled="isVersionPreview"
                                  @click="removePromptVariable(idx)"
                                >
                                  <DeleteOutlined />
                                </button>
                              </a-tooltip>
                      ...
                      <div v-if="capabilityFields.length" class="sub-config-card sub-config-card--model">
                        <div class="sub-config-card-header">
                          <div class="sub-config-card-title-row">
                            <span
                              class="capability-collapse-trigger"
                              @click="capabilityCollapsed = !capabilityCollapsed"
                            >
                              ...
                              <template v-for="field in webSearchSubFields" :key="field.key">
                                <div v-if="field.type === 'switch'" class="capability-option-item">
                                  ...
                                  <a-switch
                                    ...
                                    :disabled="isCapabilityFieldDisabled(field)"
                                  />
                                  <span class="capability-option-status">
                                    {{ agentConfig[field.key] ? '开' : '关' }}
                                  </span>
                                  <a-tooltip
                                    v-if="field.hint"
                                    :title="field.hint"
                                ...
                                <div
                                  ...
                                  class="capability-option-item capability-option-item--number"
                                >
                                  <span class="capability-option-label">{{ field.label }}</span>
                                  <a-input-number
                ...
                <a-tab-pane key="chat" tab="对话配置">
                  <div
                    class="config-tab-pane-body config-tab-pane-body--chat"
                    :class="{ 'preview-lock-zone': isVersionPreview }"
                  >
                    <a-form :model="agentConfig" :label-col="{ span: 6 }" class="panel-form">
                      ...
                      <a-form-item label="内容安全扫描" v-if="agentConfig.enableFileRead">
                        <div style="display: flex; align-items: center; gap: 8px">
                          <a-switch
                            v-model:checked="agentConfig.enableContentSecurityScan"
                            :disabled="isVersionPreview"
                          />
                          <span class="tool-option-value">
                            {{ agentConfig.enableContentSecurityScan ? '已启用' : '未启用' }}
                          </span>
                          <a-tooltip
                      ...
                      <a-form-item label="流式输出">
                        <div style="display: flex; align-items: center; gap: 8px">
                          ...
                          <span class="tool-option-value">
                            {{ agentConfig.streamOutput !== false ? '已启用' : '未启用' }}
                          </span>
                          <a-tooltip
                      ...
                      <a-form-item>
                        ...
                        <div style="display: flex; align-items: center; gap: 8px">
                          <a-switch
                            v-model:checked="agentConfig.userSensitiveFilterEnabled"
                            :disabled="isVersionPreview"
                          />
                          <span class="tool-option-value">
                            {{ agentConfig.userSensitiveFilterEnabled ? '已启用' : '未启用' }}
                          </span>
                          <a-tooltip
                      ...
                      <template v-if="agentConfig.userSensitiveFilterEnabled">
                        <a-form-item label="用户敏感词">
                          <div class="inline-field-block">
                            <button
                              type="button"
                              class="btn-add-inline btn-add-inline--block"
                              :disabled="isVersionPreview"
                              @click="addUserSensitiveWord"
                            >
                              <PlusOutlined />
                              添加敏感词
                            </button>
                            <div class="sensitive-word-list">
                              <div
                                v-for="(word, idx) in userSensitiveWords"
                                :key="'usw-' + idx"
                                class="list-table-row list-table-row--2col"
                              >
                                <a-input
                                  v-model:value="userSensitiveWords[idx]"
                                  placeholder="命中则拦截用户消息"
                                  size="small"
                                  :disabled="isVersionPreview"
                                />
                                <a-tooltip title="删除">
                                  <button
                                    ...
                                    class="btn-icon-sm danger"
                                    :disabled="isVersionPreview"
                                    @click="removeUserSensitiveWord(idx)"
                                  >
                                    <DeleteOutlined />
                                  </button>
                                </a-tooltip>
                      ...
                      <a-form-item>
                        ...
                        <div style="display: flex; align-items: center; gap: 8px">
                          ...
                          <span class="tool-option-value">
                            {{ agentConfig.sensitiveFilterEnabled ? '已启用' : '未启用' }}
                          </span>
                          <a-tooltip
                      ...
                      <template v-if="agentConfig.sensitiveFilterEnabled">
                        <a-form-item label="处理策略">
                          <a-select
                            v-model:value="agentConfig.sensitiveFilterStrategy"
                            style="width: 100%"
                            :disabled="isVersionPreview"
                          >
                        ...
                        <a-form-item v-if="agentConfig.sensitiveFilterStrategy !== 'block'" label="替换文本">
                          <a-input
                            v-model:value="agentConfig.sensitiveFilterReplaceText"
                            placeholder="默认 ***"
                            :disabled="isVersionPreview"
                          />
                        </a-form-item>
                        <a-form-item label="AI 敏感词列表">
                          <div class="inline-field-block">
                            ...
                            <div class="sensitive-word-list">
                              <div
                                v-for="(word, idx) in sensitiveWords"
                                :key="'sw-' + idx"
                                class="list-table-row list-table-row--2col"
                              >
                                <a-input
                                  v-model:value="sensitiveWords[idx]"
                                  placeholder="AI 输出命中则处理"
                                  size="small"
                                  :disabled="isVersionPreview"
                                />
                                <a-tooltip title="删除">
        ...
        >
          ...
          <template #description>
            <div class="binding-deleted-detail-row">
              <div v-for="(line, li) in deletedBindingDetailLines" :key="li" class="binding-deleted-detail-line">
                {{ line }}
              </div>
              <p class="binding-deleted-detail-hint">
                已删除的绑定运行时无法调用，已禁用的绑定需先启用才能使用。暂存或发布前将提示确认。
              </p>
            </div>
          </template>
        </a-alert>
              ...
              <div class="knowledge-bind">
                <div class="selected-knowledge">
                  <div class="selected-header">
                    ...
                    <button
                      v-if="!isVersionPreview && selectedTools.length > 0"
                      class="btn-clear"
                  ...
                  <div class="selected-knowledge-tags">
                    <div v-if="selectedTools.length === 0" class="empty-tip">暂未绑定工具，请从下方列表选择</div>
                    <div
                      v-for="t in selectedTools"
                      :key="t.id"
                      class="knowledge-tag tool-tag"
                      :class="{
                        'binding-tag--deleted': t._deleted,
                        'binding-tag--disabled': t._disabled && !t._deleted,
                      }"
                    >
                      <span class="tag-avatar" style="background: var(--gradient-entity-tool)">
                      ...
                      <span v-else class="tool-type-badge">
                        {{ toolTypeLabels[t.toolType?.code || t.toolType] || t.toolType }}
                      </span>
                ...
                <div class="knowledge-list">
                  ...
                  <div class="list-body">
                    <div
                      ...
                      :class="{
                        selected: selectedToolIds.has(toBindingId(t.id)),
                        'is-preview-locked': isVersionPreview,
                      }"
                    ...
                    >
                      <div class="item-icon tool-icon-bg">
                        ...
                        <span v-else-if="(t.toolType?.code || t.toolType) === 'builtin'" class="builtin-badge">
                          内置
                        </span>
                        <DynamicIcon :name="t.icon" :fallback="t.displayName || t.name" />
                      </div>
                    ...
                    <div v-if="filteredToolList.length === 0" class="empty-tip">暂无可用工具</div>
                  </div>
                </div>
              </div>
            </div>
                ...
                <div class="selected-knowledge">
                  <div class="selected-header">
                    <span class="selected-label">
                      已绑定 {{ selectedKnowledge.length }}/{{ BIND_LIMITS.knowledge }} 个知识库
                    </span>
                    ...
                    <button
                      v-if="!isVersionPreview && selectedKnowledge.length > 0"
                      class="btn-clear"
                      @click="clearSelectedKnowledge"
                    >
                      <DeleteOutlined />
                      清空
                    </button>
                  ...
                  <div class="selected-knowledge-tags">
                    <div v-if="selectedKnowledge.length === 0" class="empty-tip">暂未绑定知识库，请从下方列表选择</div>
                    <div
                    ...
                    >
                      <span class="tag-avatar" style="background: var(--gradient-entity-knowledge)">
                ...
                <div class="knowledge-list">
                  ...
                  <div class="list-body">
                    <div
                      ...
                      class="knowledge-item"
                      :class="{
                        selected: selectedKnowledgeIds.has(toBindingId(k.id)),
                        'is-preview-locked': isVersionPreview,
                      }"
                    ...
                    <div v-if="filteredKnowledgeList.length === 0" class="empty-tip">暂无可用知识库</div>
                  </div>
                </div>
              </div>
            ...
            <div class="binding-tab-pane" :class="{ 'preview-lock-zone': isVersionPreview }">
              <div class="knowledge-bind">
                <div class="selected-knowledge">
                  <div class="selected-header">
                    <span class="selected-label">
                      已绑定 {{ selectedMcpServers.length }}/{{ BIND_LIMITS.mcp }} 个 MCP Server
                    </span>
                    <button
                      v-if="!isVersionPreview && selectedMcpServers.length > 0"
                      class="btn-clear"
                      @click="clearSelectedMcpServers"
                    >
                      <DeleteOutlined />
                      清空
                    </button>
                  ...
                  <div class="selected-knowledge-tags">
                    ...
                    <div
                      ...
                      :class="{
                        'binding-tag--deleted': s._deleted,
                        'binding-tag--disabled': s._disabled && !s._deleted,
                      }"
                    >
                      <span class="tag-avatar" style="background: var(--gradient-entity-mcp)">
                ...
                <div class="knowledge-list">
                  ...
                  <div class="list-body">
                    <div
                      ...
                      class="knowledge-item"
                      :class="{
                        selected: selectedMcpServerIds.has(toBindingId(s.id)),
                        'is-preview-locked': isVersionPreview,
                      }"
                    ...
                    <div v-if="filteredMcpServerList.length === 0" class="empty-tip">暂无可用 MCP Server</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="binding-tab-pane" :class="{ 'preview-lock-zone': isVersionPreview }">
              <div class="subagent-bind">
                <div class="selected-subagents">
                  <div class="selected-header">
                    <span class="selected-label">
                      已绑定 {{ selectedSubAgents.length }}/{{ BIND_LIMITS.subAgent }} 个 SubAgent
                    </span>
                    <button
                      v-if="!isVersionPreview && selectedSubAgents.length > 0"
                      class="btn-clear"
                      @click="clearSelectedSubAgents"
                    >
                      <DeleteOutlined />
                      清空
                    </button>
                  ...
                  <div class="selected-subagents-tags">
                    <div v-if="selectedSubAgents.length === 0" class="empty-tip">暂未绑定 SubAgent，从下方列表选择</div>
                    <div
                    ...
                    >
                      <span class="tag-avatar" style="background: var(--gradient-entity-skill)">
                ...
                <div class="subagent-list">
                  ...
                  <div class="list-body">
                    <div
                      ...
                      class="subagent-item"
                      :class="{
                        selected: selectedSubAgentIds.has(toBindingId(s.id)),
                        'is-preview-locked': isVersionPreview,
                      }"
                    ...
                    <div v-if="filteredSubAgentList.length === 0" class="empty-tip">暂无可用 SubAgent</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div class="binding-tab-pane" :class="{ 'preview-lock-zone': isVersionPreview }">
              <div class="subagent-bind">
                <div class="selected-subagents">
                  <div class="selected-header">
                    <span class="selected-label">
                      已启用 {{ selectedSkills.length }}/{{ BIND_LIMITS.skill }} 个 Skill
                    </span>
                    <button
                      v-if="!isVersionPreview && selectedSkills.length > 0"
                      class="btn-clear"
                  ...
                  <div class="selected-subagents-tags">
                    <div v-if="selectedSkills.length === 0" class="empty-tip">暂未启用 Skill，从下方列表选择</div>
                    <div
                    ...
                    >
                      <span class="tag-avatar" style="background: var(--gradient-entity-subagent)">
                        <DynamicIcon :name="s.icon" :fallback="s.displayName || s.name" />
                      </span>
                      <span class="tag-name-wrap">
                        <span>{{ s.displayName || s.name }}</span>
                ...
                <div class="subagent-list">
                  ...
                  <div class="list-body">
                    <div
                      ...
                      class="subagent-item"
                      :class="{
                        selected: selectedSkillIds.has(toBindingId(s.id)),
                        'is-preview-locked': isVersionPreview,
                      }"
                    ...
                    <div v-if="filteredSkillList.length === 0" class="empty-tip">暂无可用 Skill</div>
                  </div>
                </div>
              </div>
            </div>
    ...
    <a-drawer v-model:open="versionDrawerVisible" placement="right" :width="480" class="agent-version-drawer">
      <template #title>
        <span>版本历史</span>
      ...
      <a-spin :spinning="versionLoading">
        ...
        >
          <div class="version-drawer-header">
            ...
            <a-tooltip v-if="idx !== 0" title="删除该版本" :overlay-style="{ maxWidth: '200px' }">
              <button
                type="button"
      ...
      <div v-if="versionPreview" class="version-preview-detail">
        ...
        <div class="preview-section">
          ...
          <div v-if="capabilityPreviewItems.length" class="preview-field-grid preview-field-grid--capabilities">
            <div v-for="item in capabilityPreviewItems" :key="item.key" class="preview-field">
        ...
        <div class="preview-section">
          ...
          <div class="preview-field-grid">
            <div v-for="row in chatConfigPreviewRows" :key="row.label" class="preview-field">
            ...
            <div class="preview-field" v-if="userSensitiveWordsPreview.length > 0">
              <label>用户敏感词列表</label>
              <div class="preview-value">
                <div v-for="(w, i) in userSensitiveWordsPreview" :key="w + '-' + i" class="preview-tag danger">
                  {{ w }}
                </div>
              </div>
            </div>
            ...
            <div class="preview-value">
              ...
              <div v-else>
                ...
                >
                  ...
                  <span v-else-if="t._disabled" class="binding-disabled-tag">已禁用</span>
                  <span v-else-if="isKnowledgeTool(t)" class="tool-knowledge-badge">知识库</span>
                  <span v-else class="preview-tag-type">
                    {{ toolTypeLabels[t.toolType?.code || t.toolType] || t.toolType }}
                  </span>
                </div>
              </div>
    ...
    <!-- 版本控制说明弹窗 -->
    <a-modal v-model:open="versionHelpVisible" title="版本控制说明" :footer="null" width="560px">
      <div class="version-help-content">
        <p>
          版本快照记录的是
          <strong>对话配置与编排数据</strong>
          ，以下字段不在版本控制范围内，修改后不会随版本回滚：
        </p>
        ...
        <p class="version-help-note">
          提示：系统提示词、变量配置、模型配置、对话配置、工具/知识库/SubAgent 绑定等均受版本控制。
        </p>
      </div>
    </a-modal>
...
const validPromptVariables = computed(() => promptVariables.value.filter((v) => v.key && /^\S+$/.test(v.key.trim())))
...
const promptPreviewHtml = computed(() => {
  ...
  return template.replace(/\{\{([^{}]+?)\}\}/g, (_, key) => {
    if (key in varMap) {
      ...
      return val
        ? `<span class="preview-replaced">${escapeHtml(val)}</span>`
        : `<span class="preview-empty">[${key}]</span>`
    }
...
const PREVIEW_ALLOW_SELECTOR =
  '.ant-tabs-nav, .ant-tabs-tab, .version-preview-banner-top, .version-preview-banner-top .link-btn'
...
const previewKnowledgeList = computed(() => {
  ...
  return resolveBindingItems(toBindingIdSet(versionPreview.value?.knowledgeIds), knowledgeList.value, {
    entityLabel: '知识库',
  })
})
...
const previewToolList = computed(() => {
  ...
  return resolveBindingItems(toBindingIdSet(versionPreview.value?.toolIds), toolList.value, { entityLabel: '工具' })
})
...
const previewMcpList = computed(() => {
  ...
  return resolveBindingItems(toBindingIdSet(versionPreview.value?.mcpServerIds), mcpServerList.value, {
...
const previewSubAgentList = computed(() => {
  ...
  return resolveBindingItems(toBindingIdSet(versionPreview.value?.subAgentIds), subAgentList.value, {
...
const previewSkillList = computed(() => {
  ...
  return resolveBindingItems(toBindingIdSet(versionPreview.value?.skillIds), skillList.value, { entityLabel: 'Skill' })
})
...
const capabilityFields = computed(() => configFields.value.filter((f) => CAPABILITY_FIELD_KEYS.has(f.key)))

const modelTuneFields = computed(() =>
  configFields.value.filter((f) => f.key !== 'modelId' && !CAPABILITY_FIELD_KEYS.has(f.key))
)

const hasCapabilitySwitches = computed(() => capabilityFields.value.some((f) => f.type === 'switch'))
...
const capabilitySwitchFields = computed(() => capabilityFields.value.filter((f) => f.type === 'switch'))
...
const chatConfigPreviewRows = computed(() => {
  ...
  if (summary.value) {
    ...
    rows.push(
      {
        label: '摘要提示词',
        text: summaryPromptVal.value ? '自定义' : '系统默认',
        fromDefault: summaryPromptVal.fromDefault,
      },
...
const capabilityPreviewItems = computed(() => {
  ...
  const webSearchOn =
    mp.enableWebSearch !== undefined
      ? !!mp.enableWebSearch
      : !!capabilityFields.value.find((f) => f.key === 'enableWebSearch')?.defaultValue
  const previewFields = [...primaryCapabilityFields.value, ...(webSearchOn ? webSearchSubFields.value : [])]
...
const deletedBindingDetailLines = computed(() => formatDeletedBindingDetailLines(deletedBindingSections.value))
    ...
    if (agentData.recommendedQuestions) {
      try {
        recommendedQuestions.value =
          typeof agentData.recommendedQuestions === 'string'
...
function confirmSaveWithDeletedBindings() {
  ...
  const title =
    hasDeleted && hasDisabled ? '存在已删除或已禁用的绑定' : hasDeleted ? '存在已删除的绑定' : '存在已禁用的绑定'
  const tipText =
    hasDeleted && hasDisabled
  ...
  return new Promise((resolve) => {
    Modal.confirm({
      ...
      content: h('div', { class: 'deleted-binding-modal' }, [
        ...
        h(
          'p',
          { class: 'deleted-binding-modal-foot' },
...
async function handleSave(options = {}) {
  ...
  const toolMetaMap = new Map(
    toolList.value.map((t) => [String(t.id), { name: t.displayName || t.name, type: t.toolType?.code || t.toolType }])
  )
  ...
  for (const sid of selectedSkillIds.value) {
    ...
    const depToolIds = Array.isArray(skill.toolIds)
      ? skill.toolIds
      : typeof skill.toolIds === 'string' && skill.toolIds
        ? JSON.parse(skill.toolIds)
        : []
  ...
  for (const sid of selectedSkillIds.value) {
    const skill = skillList.value.find((s) => String(s.id) === String(sid))
    const depMcpIds = Array.isArray(skill.mcpServerIds)
      ? skill.mcpServerIds
      : typeof skill.mcpServerIds === 'string' && skill.mcpServerIds
        ? JSON.parse(skill.mcpServerIds)
        : []
    if (!depMcpIds.length) continue
    const missing = depMcpIds.map((mid) => String(mid)).filter((mid) => !mcpIdSet.has(mid))
    if (missing.length) {
  ...
  try {
    ...
    const serializedVars = serializePromptVariables()
    const serializedUserSensitiveWords = userSensitiveWords.value.map((w) => (w || '').trim()).filter(Boolean)
...
function formatVersionTime(val) {
  ...
  const normalized = raw.includes('T') && !raw.endsWith('Z') ? raw.replace(/(\.\d{3})\d*/, '$1') : raw
...
function normalizeVersionDetailData(data) {
  ...
  const knowledgeIds = data.knowledgeIds?.length ? data.knowledgeIds : payload.knowledgeIds || payload.knowledges || []
  const toolIds = data.toolIds?.length ? data.toolIds : payload.toolIds || payload.tools || []
  const mcpServerIds = data.mcpServerIds?.length ? data.mcpServerIds : payload.mcpServerIds || payload.mcpServers || []
  const subAgentIds = data.subAgentIds?.length ? data.subAgentIds : payload.subAgentIds || payload.subagents || []
  const skillIds = data.skillIds?.length ? data.skillIds : payload.skillIds || payload.skills || []
