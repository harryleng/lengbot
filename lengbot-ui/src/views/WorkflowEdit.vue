        ...
        <div v-if="viewingTestHistory" class="test-history-viewer-overlay">
          <div class="test-history-banner">
            <span>
              {{
                testAnimating
                  ? '正在回放历史执行过程（与当前草稿可能不一致）'
                  : '正在查看历史测试快照（与当前草稿可能不一致）'
              }}
            </span>
            <a-button
              v-if="testAnimating"
              size="small"
              type="link"
              class="test-history-skip-btn"
              @click="skipTestHistoryReplay"
            >
              跳过动画
            </a-button>
          </div>
          <WorkflowViewerCanvas
            ...
            flow-id="lengbot-workflow-test-history"
...
import {
  handleLiveWorkflowTestEvent,
  applyWorkflowTestStreamResult,
  patchLocalConfirmEventsBeforeResume,
  patchLocalConfirmEventsOnAbandon,
  findSuspendNodeIdFromEvents,
} from './workflow/composables/useWorkflowTestLive.js'
...
import {
  getDefaultNodeData as buildDefaultNodeData,
  getNodeTitle as metaGetNodeTitle,
  getNodeColor as metaGetNodeColor,
  getNodeMeta,
  getNodeLibraryGroups,
  createConditionId,
} from '../views/workflow/nodeMeta'
import {
  HANDLE_IN,
  HANDLE_OUT,
  normalizeConnection,
...
const WORKFLOW_FLOW_ID = 'lengbot-workflow-edit'
...
const versionPanelStyle = computed(() => {
  ...
  const h = canvas ? Math.max(280, canvas.clientHeight - versionPanelPos.value.y - 72) : 400
  return {
...
const canDeleteDraggedNode = computed(
  () => getSelectedDeletableNodes().length > 0 || isNodeDeletable(draggingNode.value)
)
...
function getNodeDisplayName(nodeOrId) {
  const node = typeof nodeOrId === 'string' ? nodes.value.find((n) => n.id === nodeOrId) : nodeOrId
  if (!node) return '节点'
...
function formatBatchNodeHistoryLabel(action, nodeList) {
  ...
  if (names.length === 1) {
    return action === 'delete' ? `删除「${names[0]}」节点` : `移动「${names[0]}」节点位置`
  }
  if (names.length <= 3) {
    const joined = names.map((n) => `「${n}」`).join('、')
    return action === 'delete' ? `删除 ${names.length} 个节点：${joined}` : `移动 ${names.length} 个节点：${joined}`
  }
  return action === 'delete' ? `批量删除 ${names.length} 个节点` : `批量移动 ${names.length} 个节点`
}

/** 画布当前选中的可删除节点（Shift 框选 / Ctrl 多选） */
function getSelectedDeletableNodes() {
  const selectedIds = new Set((getNodes.value || []).filter((n) => n.selected).map((n) => n.id))
  if (selectedIds.size) {
...
function resolveTrashDeleteTargets(dragList) {
  ...
  return (dragList || []).map((d) => nodes.value.find((n) => n.id === d.id) || d).filter(isNodeDeletable)
}
...
function migrateWorkflowNode(node) {
  ...
  if (node.type === 'loop') {
    ...
    if (!data.output_params?.length)
      data.output_params = data.outputParams || defaults.output_params || [{ key: 'result', type: 'Object' }]
  }
  if (node.type === 'batch') {
    if (!rawData.input_params?.length && rawData.inputParams?.length) data.input_params = rawData.inputParams
    if (!rawData.output_params?.length && rawData.outputParams?.length) data.output_params = rawData.outputParams
    ...
    if (!data.output_params?.length)
      data.output_params = data.outputParams || defaults.output_params || [{ key: 'result', type: 'Array' }]
  }
  if (node.type === 'tool') {
...
function flowPointFromEvent(event) {
  try {
    ...
    /* Vue Flow 未就绪时走 DOM 回退 */
  }
  const pane =
    canvasAreaRef.value?.querySelector('.vue-flow__viewport') || canvasAreaRef.value?.querySelector('.vue-flow')
  if (pane) {
...
function applyNodeGroupMembership(nodeId, absolutePoint) {
  ...
  if (pid) {
    ...
    if (parent && isGroupNodeType(parent.type)) {
      ...
      const next = nodes.value.map((n) => (n.id === nodeId ? { ...detachFromGroup(node), position: absTopLeft } : n))
...
function copySelectedNode() {
  ...
  if (!src || isVersionPreview.value || src.type === 'start' || src.type === 'end' || isGroupBuiltinType(src.type))
    return
  ...
  if (isGroupNodeType(src.type)) {
    nodes.value
      .filter((n) => getNodeParentId(n) === src.id)
      .forEach((child, i) => {
...
function onDrop(event) {
  ...
  newNode = {
    ...
    selectable: true,
    connectable: true,
    ...(nodeType === 'input' || nodeType === 'output' ? { sourcePosition: 'right', targetPosition: 'left' } : {}),
  }
...
function onNodeDragStart({ node, nodes: draggedNodes }) {
  ...
  recordHistory(
    formatBatchNodeHistoryLabel(
      'move',
      dragList.map((d) => nodes.value.find((n) => n.id === d.id) || d)
    )
  )
...
function applyEdgePatch(edgeId, patch) {
  ...
  if (
    !isValidWorkflowConnection(merged, {
      nodes: nodes.value,
      edges: edges.value,
...
const edgeTargetCandidates = computed(() => nodes.value.filter((n) => n.type !== 'start' && !isGroupNodeType(n.type)))
  ...
  edges.value.forEach((e) => {
    ...
    if (e.targetHandle && e.targetHandle !== HANDLE_IN) {
      errors.push({
        nodeId: e.target,
        field: 'connection',
        message: `连线必须接入目标节点「入」端口，当前为 ${e.targetHandle}`,
      })
    }
...
async function abandonWorkflowTest() {
  ...
  try {
    ...
    const patched = patchLocalConfirmEventsOnAbandon({ nodeEvents: testResult.value?.nodeEvents || [] }, suspendNodeId)
    testPendingConfirm.value = null
...
.test-chat-msg.user .test-chat-content {
  background: var(--color-info-bg);
}
.test-chat-msg.assistant .test-chat-content {
  background: var(--color-canvas);
  border: 1px solid var(--color-hairline);
}
.test-chat-role {
