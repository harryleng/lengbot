<template>
  <div class="page">
    ...
    >
      <template #filters>
        <a-select v-model:value="toolTypeFilter" placeholder="工具类型" allow-clear style="width: 120px">
          <a-select-option v-for="t in toolTypeList" :key="t.value" :value="t.value">{{ t.label }}</a-select-option>
        </a-select>
        <a-select v-model:value="tagFilter" placeholder="标签筛选" allow-clear style="width: 120px">
    ...
    <a-spin :spinning="loading" style="min-height: 300px; display: block">
      <div class="provider-grid">
        <EntityCard v-for="t in list" :key="t.id" type="tool" :name="t.displayName || t.name" @click="openDetail(t)">
          <template #icon>
            <span v-if="(t.toolType?.code || t.toolType) === 'builtin'" class="type-badge badge-builtin">内置</span>
            <span v-else-if="(t.toolType?.code || t.toolType) === 'knowledge'" class="type-badge badge-knowledge">
              知识库
            </span>
          ...
          <template #actions>
            <a-tooltip
              v-if="(t.toolType?.code || t.toolType) !== 'builtin' && (t.toolType?.code || t.toolType) !== 'knowledge'"
              title="删除"
            >
              ...
              <template #overlay>
                <a-menu>
                  ...
                  <a-menu-item @click="openTestDialog(t)">
                    <PlayCircleOutlined style="margin-right: 6px" />
                    测试工具
                  </a-menu-item>
                </a-menu>
              </template>
        ...
        <a-form :model="form" :label-col="{ flex: '0 0 100px' }">
          <a-form-item label="工具标识" required>
            <a-input
              ...
              placeholder="如：http_request（英文，唯一标识）（不超过30字）"
              :maxlength="30"
              show-count
              :disabled="form.toolType === 'builtin'"
            />
          </a-form-item>
          <a-form-item label="显示名称">
            <a-input
              v-model:value="form.displayName"
              placeholder="如：HTTP 请求（不超过30字）"
              :maxlength="30"
              show-count
              :disabled="isBuiltin"
            />
          ...
          <a-form-item label="工具类型" required>
            <a-select
              v-model:value="form.toolType"
              style="width: 100%"
              :disabled="form.id && form.toolType === 'builtin'"
            >
          ...
          <template v-if="form.toolType === 'api'">
            ...
            <a-form-item v-if="form.authType && form.authType !== 'none'">
              <template #label>
                认证配置
                <a-tooltip placement="topLeft" overlay-class-name="field-tooltip-wide">
                  <template #title>
            ...
            <a-form-item>
              <template #label>
                ...
                <a-tooltip placement="topLeft" overlay-class-name="field-tooltip-wide">
                  <template #title>
                    <div>
                      定义工具返回的 JSON 结构（JSON Schema 格式）。填写后详情页会展示字段说明表，帮助用户理解工具输出。
                    </div>
                    <div class="tooltip-example-title">示例：</div>
                    <pre class="tooltip-example">
{
    ...
    "items": {
      ...
      "items": {"type": "object", "properties": {"id": {"type": "integer"}, "name": {"type": "string"}}}
              ...
              <JsonInput
                ...
                :rows="3"
                placeholder='JSON Schema，如：{"type":"object","properties":{"total":{"type":"integer","description":"结果总数"}}}'
              />
            </a-form-item>
            <a-form-item>
              <template #label>
  ...
  "items": [
    ...
    {"id": 2, "name": "文档B"}
              ...
              <JsonInput
                ...
                :rows="4"
                placeholder='示例 JSON，如：{"total":2,"results":[...]}'
              />
            </a-form-item>
      ...
      <template #footer>
        <LbDialogFooter :loading="submitting" @cancel="dialogVisible = false" @confirm="handleSubmit" />
      </template>
    </a-modal>

    <!-- 测试工具弹窗 -->
    <a-modal v-model:open="testDialogVisible" :title="testToolName || '测试工具'" :width="680" :mask-closable="false">
        ...
        <div class="dialog-scroll-body">
            ...
            <table class="test-params-table">
              ...
              <tbody>
                <tr v-for="p in testToolParams" :key="p.name">
                  ...
                  <td>{{ p.type }}</td>
                  <td>
                    <span v-if="p.required" class="param-required">是</span>
                    <span v-else class="param-optional">否</span>
                  </td>
                  <td>{{ p.desc }}</td>
                </tr>
              </tbody>
          ...
          <template v-if="testResult !== null">
            ...
            <div class="test-result">
              ...
              <pre class="test-result-content" :class="{ 'is-json': isJsonResult(testResult) }">{{
    ...
    >
              ...
              <div class="detail-info-item">
                ...
                <span class="detail-info-value">
                  <span
                    class="tag"
                    :style="{
                      background: typeColors[detailTool.toolType?.code || detailTool.toolType] + '15',
                      color: typeColors[detailTool.toolType?.code || detailTool.toolType],
                    }"
                  >
      ...
      <template #footer>
        <LbDialogFooter cancel-text="关闭" hide-confirm @cancel="detailVisible = false">
          <template #left>
            <button
