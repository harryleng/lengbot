<template>
  <div class="subagent-manage">
    ...
    <a-spin :spinning="loading" style="min-height: 300px; display: block">
      <div class="card-grid">
        <EntityCard v-for="s in list" :key="s.id" type="subagent" :name="s.displayName" @click="openDetail(s)">
          <template #icon>
        ...
        <a-form :label-col="{ flex: '0 0 100px' }">
          ...
          <a-form-item label="标识名称" required>
            <a-input
              ...
              placeholder="英文标识，如 research-agent（不超过30字）"
              :maxlength="30"
              show-count
          ...
          <a-form-item label="描述" required>
            <a-textarea
              ...
              placeholder="SubAgent 描述（不超过50字）"
              :rows="2"
              :maxlength="50"
              show-count
              :disabled="editingBuiltin"
          ...
          <a-form-item label="系统提示词" required>
            <a-textarea
              ...
              placeholder="SubAgent 的系统提示词（不超过2000字）"
              :rows="6"
              :maxlength="2000"
              show-count
              :disabled="editingBuiltin"
          ...
          <a-form-item label="模型配置">
            ...
            <span style="font-size: 13px; color: var(--color-mute)">
              {{ inheritModel ? '继承主 Agent 模型（含版本快照）' : '使用独立模型' }}
            </span>
            <ModelSelect
              v-if="!inheritModel"
    ...
    <a-modal v-model:open="guideVisible" title="SubAgent 说明" :width="640" :footer="null">
      <div class="guide">
        <div class="guide-section">
          ...
          <p>
            SubAgent 是
            <strong>专职子智能体</strong>
            ：拥有独立的系统提示词与可选工具集。主 Agent 在对话中可通过内置工具
            <code>delegate_to_subagent</code>
            将子任务委派给指定 SubAgent，子智能体在隔离上下文中完成推理与工具调用后，将结果返回主 Agent 继续回复。
          </p>
          <p>
            适用场景：代码审查、深度调研、专项写作等需要
            <strong>独立人设与工具边界</strong>
            的任务，避免主对话上下文被拉长。
          </p>
        </div>
        <div class="guide-section">
          ...
          <div class="guide-step">
            ...
            <div>
              <b>填写标识与显示名</b>
              <p>name 为英文标识（委派时引用）；displayName 为界面展示名称。</p>
            </div>
          </div>
          <div class="guide-step">
            ...
            <div>
              <b>编写描述与系统提示词</b>
              <p>描述帮助主模型判断何时委派；系统提示词定义子智能体的角色与输出规范。</p>
            </div>
          </div>
          <div class="guide-step">
            ...
            <div>
              <b>（可选）绑定工具</b>
              <p>限制 SubAgent 可调用的工具范围，不绑定则继承主 Agent 工具策略（以实现为准）。</p>
            </div>
          </div>
          <div class="guide-step">
            ...
            <div>
              <b>在 Agent 中绑定</b>
              <p>
                进入智能体详情 → SubAgents Tab，勾选要委派的子智能体（有数量上限）。保存并发布后，主 Agent
                对话即可委派。
              </p>
            </div>
          </div>
        ...
        <div class="guide-section">
          ...
          <p>
            系统预置 SubAgent 仅可调整「模型配置」（继承主 Agent
            或自选模型）与启用状态；其余字段保持系统默认，不可删除。
          </p>
        </div>
      </div>
    </a-modal>
        ...
        <LbDialogFooter cancel-text="关闭" hide-confirm @cancel="detailVisible = false">
          <template #left>
            <button
              v-if="currentDetail"
              class="lb-btn"
              @click="() => { detailVisible = false; openEditDialog(currentDetail) }"
            >
              <EditOutlined />
              编辑
            </button>
...
const editingBuiltin = computed(
  () => !!editingId.value && list.value.some((s) => s.id === editingId.value && s.isBuiltin === 1)
)
...
function parseIdArray(json) {
  ...
  try {
    return JSON.parse(json).map(String)
  } catch {
    return []
  }
}
