<template>
  <div class="eval-page">
    <LbPageTabsHeader title="评测" :tabs="tabs" :active-key="activeTab" @update:active-key="activeTab = $event">
      <template #actions>
        <a-input
        ...
        <button class="btn-outline" @click="handleRefresh" :disabled="getTabLoading(activeTab).value">
          <ReloadOutlined :spin="getTabLoading(activeTab).value" />
          刷新
        </button>
          ...
          >
            <template #bodyCell="{ column, record }">
              ...
              <template v-if="column.key === 'datasetName'">
                <a v-if="record.datasetId" @click.stop="router.push(`/app/eval/datasets/${record.datasetId}`)">
                  {{ record.datasetName || '-' }}
                </a>
                <span v-else>{{ record.datasetName || '-' }}</span>
              </template>
              <template v-if="column.key === 'promptKey'">
                <a v-if="record.promptKey" @click.stop="router.push(`/app/prompts/${record.promptKey}`)">
                  {{ record.promptKey }}
                </a>
                <span v-else>-</span>
              </template>
              ...
              <template v-if="column.key === 'evaluatorName'">
                <template v-if="record.evaluatorNameList?.length">
                  <template v-for="(name, i) in record.evaluatorNameList.slice(0, 2)" :key="name + '-' + i">
                    <a
                      v-if="record.evaluatorIdList?.[i]"
                      @click.stop="router.push(`/app/eval/evaluators/${record.evaluatorIdList[i]}`)"
                    >
                      {{ name }}
                    </a>
                    <span v-else>{{ name }}</span>
                    <span v-if="i < Math.min(1, record.evaluatorNameList.length - 1)" style="color: var(--color-mute)">
                      、
                    </span>
                  </template>
                  <a-tooltip v-if="record.evaluatorNameList.length > 2" :title="record.evaluatorNameList.join('、')">
                    <span style="color: var(--color-mute); cursor: pointer">
                      ...等{{ record.evaluatorNameList.length }}个
                    </span>
                  </a-tooltip>
                </template>
              ...
              <template v-if="column.key === 'action'">
                <div class="table-actions">
                  <a-tooltip v-if="(record.status?.code || record.status) === 'running'" title="停止">
                    <button class="btn-icon" @click="handleStopExperiment(record.id)">
                      <PauseCircleOutlined />
                    </button>
                  </a-tooltip>
                  <a-tooltip v-if="(record.status?.code || record.status) === 'stopped'" title="重启">
                    <button class="btn-icon" @click="handleRestartExperiment(record.id)">
                      <PlayCircleOutlined />
                    </button>
                  </a-tooltip>
                  <a-tooltip title="删除">
                    <button class="btn-icon danger" @click="handleDeleteExperiment(record.id)">
                      <DeleteOutlined />
                    </button>
                  </a-tooltip>
                </div>
      ...
      <a-form :model="datasetForm" :label-col="{ flex: '0 0 100px' }">
        ...
        <a-form-item label="描述">
          <a-textarea
            ...
            :maxlength="50"
            show-count
            placeholder="评测集的用途描述 (不超过50字)"
          />
        </a-form-item>
      </a-form>
      <template #footer>
        <LbDialogFooter :loading="submitting" @cancel="datasetDialogVisible = false" @confirm="handleDatasetSubmit" />
      </template>
    </a-modal>
    ...
    >
      <a-form :model="evaluatorForm" :label-col="{ flex: '0 0 100px' }">
        <a-form-item label="名称" required>
          <a-input
            v-model:value="evaluatorForm.name"
            :maxlength="30"
            show-count
            placeholder="如：准确性评估器 (不超过30字)"
          />
        </a-form-item>
        <a-form-item label="描述">
          <a-textarea
            v-model:value="evaluatorForm.description"
            :rows="3"
            :maxlength="50"
            show-count
            placeholder="评估器的用途描述 (不超过50字)"
          />
        </a-form-item>
    ...
    <!-- 示例评测集弹窗 -->
    <a-modal v-model:open="datasetExampleVisible" title="示例评测集" :width="640" :footer="null" :mask-closable="false">
          ...
          <div v-for="ex in datasetExamples" :key="ex.key" class="example-card">
            <div class="example-card-header">
              ...
              <a-button
                ...
                @click="handleCreateDatasetExample(ex.key)"
              >
                生成
              </a-button>
            </div>
          ...
          <div v-for="ex in evaluatorExamples" :key="ex.key" class="example-card">
            <div class="example-card-header">
              ...
              <a-button
                ...
                size="small"
                :loading="exampleCreating === ex.key"
                @click="handleCreateEvaluatorExample(ex.key)"
              >
                生成
              </a-button>
            </div>
      ...
      <div class="dialog-scroll-body">
        ...
        <a-form v-show="experimentStep === 0" :model="experimentForm" :label-col="{ flex: '0 0 100px' }">
          <a-form-item label="实验名称" required>
            <a-input
              v-model:value="experimentForm.name"
              :maxlength="30"
              show-count
              placeholder="如：客服 Prompt v1 vs v2 对比 (不超过30字)"
            />
          </a-form-item>
          <a-form-item label="描述">
            <a-textarea
              v-model:value="experimentForm.description"
              :rows="3"
              :maxlength="50"
              show-count
              placeholder="实验目的说明 (不超过50字)"
            />
          </a-form-item>
        </a-form>
        ...
        <a-form v-show="experimentStep === 1" :model="experimentForm" :label-col="{ flex: '0 0 100px' }">
          ...
          <a-form-item label="数据版本" required>
            <a-select v-model:value="experimentForm.datasetVersion" placeholder="选择数据版本" style="width: 100%">
        ...
        <a-form v-show="experimentStep === 2" :model="experimentForm" :label-col="{ flex: '0 0 100px' }">
          ...
          <a-form-item label="Prompt 版本" required>
            <a-select v-model:value="experimentForm.promptVersion" placeholder="选择版本" style="width: 100%">
        ...
        <div v-show="experimentStep === 3">
          <div
            v-for="(ev, idx) in experimentForm.evaluators"
            :key="ev.evaluatorId || ev.id || idx"
            class="evaluator-config-block"
          >
            ...
            <a-form :model="ev" :label-col="{ flex: '0 0 100px' }">
              <a-form-item label="评估器" required>
                <a-select
                  ...
                  placeholder="选择评估器"
                  style="width: 100%"
                  @change="(id) => onExpEvaluatorChange(idx, id)"
                >
              ...
              <a-form-item label="参数映射">
                <a-textarea
                  ...
                  :rows="2"
                  placeholder='JSON: {"actual_output":"output","expected_output":"reference"}'
                />
              </a-form-item>
            </a-form>
          </div>
          <a-button
            v-if="experimentForm.evaluators.length < 5"
            type="dashed"
            size="small"
            block
            @click="addExpEvaluator"
          ...
          <div
            v-if="experimentForm.evaluators.length >= 5"
            style="margin-top: 4px; font-size: 12px; color: var(--color-mute); text-align: center"
          >
            最多添加5个评估器
          </div>
        </div>
...
import {
  ...
  updateEvalDataset,
  deleteEvalDataset,
  listEvalDatasetExamples,
  createFromEvalDatasetExample,
...
import {
  ...
  updateEvaluator,
  deleteEvaluator,
  listEvaluatorExamples,
  createFromEvaluatorExample,
...
async function openDatasetExampleModal() {
  ...
  try {
    ...
    datasetExamples.value = res.data || []
  } catch {
    datasetExamples.value = []
  }
}
...
async function handleEvaluatorSubmit() {
  ...
  try {
    if (evaluatorForm.id) {
      await updateEvaluator(evaluatorForm.id, {
        name: evaluatorForm.name,
        description: evaluatorForm.description,
        tags: evaluatorForm.tags,
      })
      message.success('更新成功')
    } else {
...
async function openEvaluatorExampleModal() {
  ...
  try {
    ...
    evaluatorExamples.value = res.data || []
  } catch {
    evaluatorExamples.value = []
  }
}
...
async function onExpDatasetChange(id) {
  ...
  try {
    ...
    expDatasetVersions.value = res.data || []
  } catch {
    expDatasetVersions.value = []
  }
}
...
async function onExpPromptChange(key) {
  ...
  try {
    ...
    expPromptVersions.value = res.data || []
  } catch {
    expPromptVersions.value = []
  }
}
...
async function onExpEvaluatorChange(idx, id) {
  ...
  try {
    ...
    experimentForm.evaluators[idx].versions = res.data || []
  } catch {
    experimentForm.evaluators[idx].versions = []
  }
}
...
function nextExperimentStep() {
  ...
  if (experimentStep.value === 1 && (!experimentForm.datasetId || !experimentForm.datasetVersion))
    return message.warning('请选择评测集和版本')
  if (experimentStep.value === 2 && (!experimentForm.promptKey || !experimentForm.promptVersion))
    return message.warning('请选择 Prompt 和版本')
  ...
  try {
    ...
    if (experimentForm.variableMapping.trim()) {
      try {
        ...
        variableMap = Object.entries(parsed).map(([promptVariable, datasetColumn]) => ({
          promptVariable,
          datasetColumn,
        }))
      } catch {
        return message.warning('变量映射 JSON 格式不正确')
      }
    }
    ...
    const evaluatorConfigArr = experimentForm.evaluators.map((ev) => {
      ...
      if (ev.evaluatorParamMapping?.trim()) {
        try {
          ...
          evaluatorParamMap = Object.entries(parsed).map(([evaluatorVariable, source]) => ({
            evaluatorVariable,
            source,
          }))
        } catch {
          throw new Error('评估器参数映射 JSON 格式不正确')
        }
      }
...
.evaluator-remove-btn {
  color: var(--color-mute);
  cursor: pointer;
  font-size: 14px;
}
.evaluator-remove-btn:hover {
  color: #ef4444;
}
</style>
