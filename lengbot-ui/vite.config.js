      ...
      '/api': {
        ...
        configure: (proxy) => {
          proxy.on('proxyReq', (proxyReq, req) => {
            // ── Origin 规范化（关键）──────────────────────────────────────
            // 浏览器 → vite(5174) 是同源，vite → 后端(8082) 是服务端转发，
            // 本质不存在跨域。但 http-proxy 默认会把浏览器的 Origin 原样透传，
            // 导致用局域网 IP（如 http://10.10.13.163:5174）访问时，后端
            // CorsFilter 白名单（只有 localhost:5173/5174/3000）判定
            // "Invalid CORS request" → 403，前端表现为登录时提示"无权访问"。
            //
            // 这里统一改写成后端白名单内的 origin，从此不管用 localhost、
            // 局域网 IP 还是自定义 host 访问 dev server，都不会再触发 CORS 403。
