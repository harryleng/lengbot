...
public class ToolEventGenerator {
    ...
    public String errorRetryEvent(String message, String code, int attempt, int maxRetries) {
        return errorRetryEvent(message, code, attempt, maxRetries, false);
    }

    /**
     * 模型调用重试事件。
     *
     * @param resetContent 是否为“中途断流”重试：true 表示本轮已向前端吐过部分正文，
     *                     前端须先清空已展示内容再接收重传，否则重跑内容会接在旧内容后面。
     *                     典型场景：DeepSeek 等上游在 SSE 流式输出中途 Connection reset。
     */
    public String errorRetryEvent(String message, String code, int attempt, int maxRetries, boolean resetContent) {
        try {
            Map<String, Object> evt = new java.util.LinkedHashMap<>();
            ...
            evt.put("resetContent", resetContent);
            return objectMapper.writeValueAsString(evt);
        } catch (Exception e) {
