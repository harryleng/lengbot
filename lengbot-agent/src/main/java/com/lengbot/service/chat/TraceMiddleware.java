...
public class TraceMiddleware implements ChatMiddleware {
                        ...
                        if (ctx.getToolCallbackMap() != null && !ctx.getToolCallbackMap().isEmpty()) {
                            List<Map<String, Object>> toolDefs = ctx.getToolCallbackMap().values().stream()
                                    .map(cb -> {
                                        ...
                                        td.put("name", cb.getName());
                                        td.put("description", cb.getDescription());
                                        td.put("inputSchema", cb.getParameters());
                                        return td;
                                    })
                                    .toList();
    ...
    private void persistTrace(ChatContext ctx, String status, long durationMs, String errorMessage) {
        ...
        trace.setReplyContent(com.lengbot.util.TextNormalizeUtil.sanitizeForDatabase(completeReply));
        // 最终展示：与用户对话页正文一致（剥离 thinking 后）
        trace.setDisplayContent(com.lengbot.util.TextNormalizeUtil.sanitizeForAiMessage(
                ctx.getFullReply().toString(), 0));
        trace.setErrorMessage(com.lengbot.util.TextNormalizeUtil.sanitizeForDatabase(errorMessage));
        try {
