...
import com.lengbot.dto.KnowledgeSaveDTO;
import com.lengbot.dto.DifyKnowledgeConfigDTO;
import com.lengbot.entity.Document;
import com.lengbot.entity.Knowledge;
import org.springframework.util.StringUtils;
import com.lengbot.entity.KnowledgeMember;
...
import com.lengbot.service.GraphService;
...
import com.lengbot.util.MindmapUtil;
import com.lengbot.util.DifySecretCipher;
import com.lengbot.vo.DifyConnectionTestVO;
...
public class KnowledgeServiceImpl extends ServiceImpl<KnowledgeMapper, Knowledge>
    ...
    public Knowledge create(KnowledgeSaveDTO request) {
        ...
        if (knowledge.getType() == KnowledgeType.DIFY) {
            ...
            knowledgeRetrievalService.retrieve(knowledge, "LengBot connection test", 1, 0.0, Map.of());
        }
    ...
    public Knowledge update(KnowledgeSaveDTO request) {
        ...
        if (existing.getType() != KnowledgeType.DIFY) {
            ...
            knowledgeRetrievalService.retrieve(existing, "LengBot connection test", 1, 0.0, Map.of());
        }
        updateById(existing);
    ...
    public void generateExampleQuestions(Long knowledgeId, Long documentId) {
        ...
        try {
            ...
            messages.add(Msgs.system(QUESTION_GEN_SYSTEM_PROMPT));
    ...
    public String generateOneExampleQuestion(Long knowledgeId) {
        ...
        try {
            var response = LlmTraceContext.callWithoutTrace(() -> ModelCalls.call(model, messages));
            reply = Msgs.extractText(response);
    ...
    public DifyConnectionTestVO testDifyConnection(DifyKnowledgeConfigDTO config) {
        ...
        difyDatasetClient.retrieve(config.getApiUrl().trim(), config.getDatasetId().trim(), config.getToken().trim(),
                "LengBot connection test", 1, 0.0, "vector");
