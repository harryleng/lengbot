...
public class ImageGenTool {
    ...
    @Value("${lengbot.siliconflow.api-key:}")
    ...
    public String generate(
            @ToolParam(name = "prompt", description = "图片描述（英文效果更佳）")
            @ToolParamMeta(example = "a beautiful sunset over the ocean") String prompt,
            @ToolParam(name = "negativePrompt", description = "负面提示词，描述不希望出现的元素（可选）", required = false)
        ...
        if (apiKey == null || apiKey.isBlank()) {
            return "图片生成未配置，请在配置文件中设置 lengbot.siliconflow.api-key";
        }
    ...
    private Long extractSessionId(ToolCallParam toolContext) {
