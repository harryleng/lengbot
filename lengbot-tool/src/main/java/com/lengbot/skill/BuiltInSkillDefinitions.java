...
public final class BuiltInSkillDefinitions {
    ...
    private static final List<Definition> DEFS = List.of(
            ...
            new Definition(
                    ...
                    **LengBot 工具约束**：`calculator` 一次只接受 `a`、`b` 和 `operation`，其中 `operation` 只能是 `add`、`subtract`、`multiply`、`divide`。
            ...
            new Definition(
                    ...
                    "图片创作",
                    "PictureOutlined",
                    "面向需要生成插画、海报、示意图等场景，基于明确的画面要求调用 LengBot 图像生成工具。",
                    ...
                    ## 可用工具
                    - `image_generation(prompt, negativePrompt?)`：LengBot 内置文生图工具，底层已封装 SiliconFlow / Qwen-Image，会把生成图片保存到本会话输出区并返回真实 `image_url`。不支持参考图编辑、尺寸选择或批量生成。
            ...
            new Definition(
                    ...
                    根据用户的指令，通过内置只读工具访问 LengBot 的 PostgreSQL 业务库，并结合图表工具（若已绑定）构建 SQL 查询报告。
