...
public interface McpClientService {
     ...
     *
     * @param mcpServerId MCP Server ID
     * @return ToolBase 列表（已过滤 disabled_tools）
     */
    List<ToolBase> getToolCallbacks(Long mcpServerId);

    /**
     * 获取 MCP Server 的所有工具回调（不过滤 disabled_tools）
     *
     * @param mcpServerId MCP Server ID
     * @return ToolBase 列表（全量）
     */
    List<ToolBase> getAllToolCallbacks(Long mcpServerId);
