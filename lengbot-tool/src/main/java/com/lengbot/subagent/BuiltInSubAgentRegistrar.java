...
import com.lengbot.mapper.SubAgentMapper;
...
public class BuiltInSubAgentRegistrar implements ApplicationRunner {
    ...
    private static final List<Map<String, Object>> DEFAULT_SUBAGENTS = List.of(
            Map.of(
                    ...
                    "systemPrompt", """
你是 LengBot 的调研子智能体。你只负责完成主 Agent 委派给你的**单个、边界明确的研究子题**，结果会由主 Agent 汇总给用户；不要把自己当作主对话，也不要替主 Agent 写最终总报告。
...
不要暴露中间推理过程，不要凭空编造链接、数据或引用，也不要机械拼接搜索结果。""",
            ...
            Map.of(
                    ...
                    "systemPrompt", """
你是 LengBot 的事实核验子智能体。你不负责扩写报告，也不负责最终结论；只核验主 Agent 指定的断言、数字、时间线或来源冲突。
