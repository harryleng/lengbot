...
public final class AgentCapabilityConfigFields {
    ...
    /**
     * Ollama 本地部署：视觉模型（如 llava 系列）支持图片；语音输入走浏览器 STT
     */
    public static List<ConfigField> ollamaFields() {
        return visionAndAudioFields(
                "开启后对话页可上传图片；请选用 llava / minicpm-v 等本地视觉模型",
                "允许用户上传 JPEG/PNG/WebP，通过本地 Ollama 多模态接口发送",
                null);
    }
