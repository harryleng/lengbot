-- lengbot 2.1.0 init SQL
-- Generated by merging sql/*.sql through 2026-09-01.
...
-- ============================================================
-- lengbot Database Initialization (Complete)
-- PostgreSQL 15 + pgvector
-- 合并日期：2026-09-01
-- 包含 v2.1.0 的最终结构；存量环境升级请执行后续增量迁移文件
-- ============================================================

CREATE DATABASE lengbot ENCODING 'UTF8';
\c lengbot;
...
-- ========================================
-- TTS 音色管理表
-- 缓存各 Provider（如 edge-tts）返回的音色清单，并叠加收藏/分组/备注等本地管理字段，
-- 使音色列表可离线使用、可按需整理，而不必每次实时拉取 Provider。
-- ========================================
...
CREATE UNIQUE INDEX uk_tts_voice_provider_name ON tts_voice (provider, voice_name) WHERE deleted = 0;
CREATE INDEX idx_tts_voice_locale ON tts_voice (locale);
CREATE INDEX idx_tts_voice_gender ON tts_voice (gender);
CREATE INDEX idx_tts_voice_favorite ON tts_voice (favorite);
CREATE INDEX idx_tts_voice_group ON tts_voice (voice_group);
...
VALUES (
    1, 1, 'lengbot 助手', '默认AI助手', 'chat',
    '你是 lengbot 智能助手，请用中文回答用户问题。回答应简洁准确，遇到不确定的信息请如实告知。',
    '## 你好，我是 lengbot
有什么可以帮你的？',
...
INSERT INTO system_config (config_key, config_value, description) VALUES (
  ...
  '{
    ...
    "description": "构建智能体、知识库、工作流与工具集成的统一平台。从 Prompt 工程到 RAG 检索增强，从 Workflow 编排到 MCP 工具生态，lengbot 为 AI 应用开发提供全栈能力支撑。",
    "features": [
...
-- 评估器表新增 tags 字段（base 快照已含该列，幂等化避免重复执行报错）
ALTER TABLE eval_evaluator ADD COLUMN IF NOT EXISTS tags VARCHAR(200);
COMMENT ON COLUMN eval_evaluator.tags IS '标签，逗号分隔';

-- 实验结果表新增 evaluator_name 字段
ALTER TABLE eval_experiment_result ADD COLUMN IF NOT EXISTS evaluator_name VARCHAR(128);
...
ALTER TABLE tool ADD COLUMN IF NOT EXISTS output_example JSONB DEFAULT '{}';
...
ALTER TABLE chat_session ADD COLUMN IF NOT EXISTS agent_version_id BIGINT;
...
ALTER TABLE message ADD COLUMN IF NOT EXISTS reply_to_message_id BIGINT;
...
ALTER TABLE mcp_server ADD COLUMN IF NOT EXISTS last_sync_time TIMESTAMP;
...
ALTER TABLE api_key ADD COLUMN IF NOT EXISTS agent_ids JSONB DEFAULT NULL;
ALTER TABLE api_key ADD COLUMN IF NOT EXISTS rate_limit INT NOT NULL DEFAULT 60;
ALTER TABLE api_key ADD COLUMN IF NOT EXISTS daily_quota INT NOT NULL DEFAULT 100000;
ALTER TABLE api_key ADD COLUMN IF NOT EXISTS used_tokens BIGINT NOT NULL DEFAULT 0;
ALTER TABLE api_key ADD COLUMN IF NOT EXISTS quota_reset_at DATE DEFAULT NULL;
